...
if ~exist('co2')
  co2 = zeros(1,iMaxTimeSteps);
  n2o = zeros(1,iMaxTimeSteps);
  ch4 = zeros(1,iMaxTimeSteps);
  cfc11 = zeros(1,iMaxTimeSteps);
  cfc12 = zeros(1,iMaxTimeSteps);
  stemp = zeros(1,iMaxTimeSteps);

  co2_sig = zeros(1,iMaxTimeSteps);
  n2o_sig = zeros(1,iMaxTimeSteps);
  ch4_sig = zeros(1,iMaxTimeSteps);
  cfc11_sig = zeros(1,iMaxTimeSteps);
  cfc12_sig = zeros(1,iMaxTimeSteps);
  stemp_sig = zeros(1,iMaxTimeSteps);

  dofs  = zeros(1,iMaxTimeSteps);
  %cdofs = zeros(1,iMaxTimeSteps,66);
  %ak = zeros(1,iMaxTimeSteps,66,66);
end
...
for ii = iReadLatbin
  ...
  for iTime = 1 : iMaxTimeSteps
    ...
    elseif exist(fname) & iaFound(mapp(iTime)) == 0
      loader = ['a = load(''' fname ''');'];
...
fprintf(1,'expect about iMaxTimeSteps files \n')
...
if length(baddy) > 0
  ...
  fprintf(1,'found that %3i of %3i timesteps did not finish : see badanom.sc \n',length(baddy),iMaxTimeSteps)
