...
for iLatbin = iLS : iLE
  ...
  for iChan = 1 : 1305
    ...
    for iDay16 = 1 : length(ia16)-1
      ...
      avg16_btanom(iDay16,iChan) = nanmean(ysmooth(ind)) * 1000;
  ...
  if i_obsORcal > 0
    fsave = ['ANOM_16dayavg/latbin_' num2str(iAvgNumDays) 'dayavg_' num2str(iLatbin) '.mat'];
    saver = ['save ' fsave ' avg16_rtime avg_doy_since2012 avg16_btanom comment'];
  elseif i_obsORcal < 0
    fsave = ['ANOM_16dayavg/latbin_' num2str(iAvgNumDays) 'dayavg_' num2str(iLatbin) '_cal.mat'];
    saver = ['save ' fsave ' avg16_rtime avg_doy_since2012 avg16_btanom comment'];
  end
