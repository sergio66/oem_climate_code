...
if ~exist('co2')
  ...
  bt1231_signal     = zeros(1,40);
  bt1231_signal_unc = zeros(1,40);
  bt1231_fit        = zeros(1,40);
  t850_fit          = zeros(1,40);

  bt_signal  = zeros(1305,40);
  bt_fit     = zeros(1305,40);
  ...
  for iTime = iReadTimeStep
    ...
    elseif exist(fname) > 0
      ...
      if co2lays == 1
        ...
        bt1231_signal(ii)     = a.rateset.rates(i1231);
        bt1231_signal_unc(ii) = a.rateset.unc_rates(i1231);
        bt1231_fit(ii)        = a.oem.fit(i1231);
        t850_fit(ii)          = a.oem.finalrates(tzind(19));

        bt_signal(:,ii) = a.rateset.rates;
        bt_fit(:,ii)    = a.oem.fit;
...
iaTropics = find(latbins >= -30 & latbins <= 30);
...
figure(1); clf; errorbar(1:40,bt1231_signal,bt1231_signal_unc,'color','b'); 
           hold on; plot(1:40,bt1231_fit,'r',1:40,stemp,'k',1:40,t850_fit,'g'); hold off
           ylabel('BT 1231(K)'); title('(b)signal (r)fit (k)stemp (g)T850mb');
...
for ii = 1 : 6
  ...
  ax = axis; line([iaTropics(1)   iaTropics(1)],[ax(3) ax(4)],'color','k','linewidth',2)
  ax = axis; line([iaTropics(end) iaTropics(end)],[ax(3) ax(4)],'color','k','linewidth',2)
end
...
if iDoDOFS > 0
  ...
  figure(11)
  pcolor(1:38,playsN,tz'); colormap(usa2); caxis([-1 +1]); title('T(z,lat)');
...
%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  figure(11); 
  pcolor(1:40,f(chanset),bt_signal(chanset,:)); colormap(usa2); caxis([-2 +2]); title('Signal(z,f)');
    shading interp; set(gca,'ydir','reverse'); colorbar
  ax = axis; line([iaTropics(1)   iaTropics(1)],[ax(3) ax(4)],'color','k','linewidth',2)
  ax = axis; line([iaTropics(end) iaTropics(end)],[ax(3) ax(4)],'color','k','linewidth',2)
