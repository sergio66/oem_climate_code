...
for JOB = 1 : 64
  ...
  %}
    ...
    savedir = ['AVGJAC64/Latbin' num2str(JOB,'%02d')];
    if ~exist(savedir)
      makedir = ['!mkdir ' savedir];
      eval(makedir);
    end
    theppmv.ppmv2 = kcarta.subjac.ppmv2;
    theppmv.ppmv4 = kcarta.subjac.ppmv4;
    theppmv.ppmv6 = kcarta.subjac.ppmv6;
    saver = ['save ' savedir '/m_ts_jac_latbin' num2str(JOB,'%02d') '_lonbin' num2str(lon,'%02d') '.mat m_ts_jac_fast nlays theppmv freq2645'];
    eval(saver);
  end
  fprintf(1,'\n')
