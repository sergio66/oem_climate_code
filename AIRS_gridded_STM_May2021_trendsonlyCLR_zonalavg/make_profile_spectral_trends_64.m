...
umbc_spectral_rates     = zeros(2645,64);
era_spectral_rates      = zeros(2645,64);
era5_spectral_rates     = zeros(2645,64);
airsL3_spectral_rates   = zeros(2645,64);
...
%for ii = 32
  ...
  clear x100m_ts
  jacfilex = ['AVGJAC64/Latbin' num2str(ii,'%02d') '/avg_72bins_m_ts_jac_latbin' num2str(ii,'%02d') '.mat'];
  jacx = load(jacfilex);    
    %% THESE ARE NORMALIZED JACS   JTRUE*RENORM
    m_ts_jac.subjac.coljacCO2   = jacx.m_ts_jac_fast(:,1)*2.2/400*TGmult;
    m_ts_jac.subjac.coljacN2O   = jacx.m_ts_jac_fast(:,2)*0.8/300*TGmult;
    m_ts_jac.subjac.coljacCH4   = jacx.m_ts_jac_fast(:,3)*5.0/1860*TGmult;
    m_ts_jac.subjac.coljacCFC11 = jacx.m_ts_jac_fast(:,4)*0;
    m_ts_jac.subjac.coljacCFC12 = jacx.m_ts_jac_fast(:,5)*0;
    m_ts_jac.subjac.jacST       = jacx.m_ts_jac_fast(:,6)*0.1;
    nlays = jacx.nlays+1;
    wah = ((1:jacx.nlays)+6+jacx.nlays*0); boo = zeros(100,2645); boo(1:jacx.nlays,:) =  jacx.m_ts_jac_fast(:,wah)'*0.01;         x100m_ts_jac.subjac.jacWV = boo;
    wah = ((1:jacx.nlays)+6+jacx.nlays*1); boo = zeros(100,2645); boo(1:jacx.nlays,:) =  jacx.m_ts_jac_fast(:,wah)'*0.01;         x100m_ts_jac.subjac.jacT  = boo;
    wah = ((1:jacx.nlays)+6+jacx.nlays*2); boo = zeros(100,2645); boo(1:jacx.nlays,:) =  jacx.m_ts_jac_fast(:,wah)'*0.01*TGmult;  x100m_ts_jac.subjac.jacO3 = boo;
...
%  m_ts_jac.subjac.ppmv6 = jac.subjac.ppmv6;
  ...
  m_ts_jac.subjac.jacWV = squeeze(quick_combinejaclays_make_profile_spectral_trends(x100m_ts_jac.subjac.jacWV * 1.00,1:100,20))';
  m_ts_jac.subjac.jacT  = squeeze(quick_combinejaclays_make_profile_spectral_trends(x100m_ts_jac.subjac.jacT  * 1.00,1:100,20))';
  m_ts_jac.subjac.jacO3 = squeeze(quick_combinejaclays_make_profile_spectral_trends(x100m_ts_jac.subjac.jacO3 * 1.00,1:100,20))';
  ...
  junkrate = era.trend_ptemp(:,ind) + xNPMerror*era.trend_ptemp_unc(:,ind); junkrate(isnan(junkrate)) = 0; 
    for jjj = 1 : 100; era_spectral_rates(:,ind) = era_spectral_rates(:,ind) + x100m_ts_jac.subjac.jacT(jjj,:)' * junkrate(jjj)/0.01; end
  ...
  junkrate = era.trend_gas_1(:,ind) + xNPMerror*era.trend_gas_1_unc(:,ind); junkrate(isnan(junkrate)) = 0; 
    for jjj = 1 : 100; era_spectral_rates(:,ind) = era_spectral_rates(:,ind) + x100m_ts_jac.subjac.jacWV(jjj,:)' * junkrate(jjj)/0.01 * wvmult.eraORcmip6; end
  ...
  junkrate = era.trend_gas_3(:,ind) + xNPMerror*era.trend_gas_3_unc(:,ind); junkrate(isnan(junkrate)) = 0; 
    for jjj = 1 : 100; era_spectral_rates(:,ind) = era_spectral_rates(:,ind) + x100m_ts_jac.subjac.jacO3(jjj,:)' * junkrate(jjj)/0.01; end
  ...
  if iERAorCMIP6 > 0
    ...
    junkrate = era5.trend_ptemp(:,ind) + xNPMerror*era5.trend_ptemp_unc(:,ind); junkrate(isnan(junkrate)) = 0; 
      for jjj = 1 : 100; era5_spectral_rates(:,ind) = era5_spectral_rates(:,ind) + x100m_ts_jac.subjac.jacT(jjj,:)' * junkrate(jjj)/0.01; end
    ...
    junkrate = era5.trend_gas_1(:,ind) + xNPMerror*era5.trend_gas_1_unc(:,ind); junkrate(isnan(junkrate)) = 0; 
      for jjj = 1 : 100; era5_spectral_rates(:,ind) = era5_spectral_rates(:,ind) + x100m_ts_jac.subjac.jacWV(jjj,:)' * junkrate(jjj)/0.01 * wvmult.era5; end
    ...
    junkrate = era5.trend_gas_3(:,ind) + xNPMerror*era5.trend_gas_3_unc(:,ind); junkrate(isnan(junkrate)) = 0; 
      for jjj = 1 : 100; era5_spectral_rates(:,ind) = era5_spectral_rates(:,ind) + x100m_ts_jac.subjac.jacO3(jjj,:)' * junkrate(jjj)/0.01; end
    ...
    xjunkrate = airsL3.trend_ptemp(:,ind) + xNPMerror*airsL3.trend_ptemp_unc(:,ind); xjunkrate = squeeze(xjunkrate); clear junkrate
      ...
      for jjj = 1 : 100; airsL3_spectral_rates(:,ind) = airsL3_spectral_rates(:,ind) + x100m_ts_jac.subjac.jacT(jjj,:)' * junkrate(jjj)/0.01; end
    ...
    xjunkrate = airsL3.trend_gas_1(:,ind) + xNPMerror*airsL3.trend_gas_1_unc(:,ind); xjunkrate = squeeze(xjunkrate); clear junkrate
      ...
      junkrate = junkrate'; junkrate(isnan(junkrate)) = 0; 
      for jjj = 1 : 100; airsL3_spectral_rates(:,ind) = airsL3_spectral_rates(:,ind) + x100m_ts_jac.subjac.jacWV(jjj,:)' * junkrate(jjj)/0.01 * wvmult.airsL3; end
    ...
    xjunkrate = airsL3.trend_gas_3(:,ind) + xNPMerror*airsL3.trend_gas_3_unc(:,ind); xjunkrate = squeeze(xjunkrate); clear junkrate
      junkrate = interp1(log(Tlevs),xjunkrate,log(plays),[],'extrap'); jjj = find(plays <= min(Tlevs) | plays >= max(Tlevs)); junkrate(jjj) = 0;
      junkrate = junkrate'; junkrate(isnan(junkrate)) = 0; 
      for jjj = 1 : 100; airsL3_spectral_rates(:,ind) = airsL3_spectral_rates(:,ind) + x100m_ts_jac.subjac.jacO3(jjj,:)' * junkrate(jjj)/0.01; end
    ...
    xjunkrate = resultsT(ind,:) + xNPMerror*resultsAIRS_unc.resultsT_err(ind,:); clear junkrate
      junkrate = xjunkrate; 
      junkrate(isnan(junkrate)) = 0; for jjj = 1 : 100/5; umbc_spectral_rates(:,ind) = umbc_spectral_rates(:,ind) + m_ts_jac.subjac.jacT(:,jjj) * junkrate(jjj)/0.01; end
      umbc_20_layertrends.gas_3(:,ind) = junkrate;
    xjunkrate = resultsWV(ind,:) + xNPMerror*resultsAIRS_unc.resultsWV_err(ind,:); clear junkrate
      junkrate = xjunkrate; 
      junkrate(isnan(junkrate)) = 0; for jjj = 1 : 100/5; umbc_spectral_rates(:,ind) = umbc_spectral_rates(:,ind) + m_ts_jac.subjac.jacWV(:,jjj) * junkrate(jjj)/0.01 * wvmult.umbc; end
      umbc_20_layertrends.gas_3(:,ind) = junkrate;
    xjunkrate = resultsO3(ind,:) + xNPMerror*resultsAIRS_unc.resultsO3_err(ind,:); clear junkrate
      junkrate = xjunkrate;
      junkrate(isnan(junkrate)) = 0; for jjj = 1 : 100/5; umbc_spectral_rates(:,ind) = umbc_spectral_rates(:,ind) + TGmult*m_ts_jac.subjac.jacO3(:,jjj) * junkrate(jjj)/0.01; end
      umbc_20_layertrends.gas_3(:,ind) = junkrate;
  end
end
