...
figure(1); clf
  pcolor(latx,playsRET,wv_ret*10); shading flat
...
figure(2); clf
  pcolor(latx,playsRET,temp_ret*10); shading flat;
  caxis([-0.15 +0.15]); colorbar; title('T(lat,z)/decade')
  caxis([-0.50 +0.50]); colorbar; title('T(lat,z)/decade')

figure(3); clf
  pcolor(latx,playsRET,ozone_ret*10); shading flat
...
figure(7); clf
  plot(latx,traceNstemp(1:5,:),latx,traceNstemp(6,:)*10,'g','linewidth',2); grid
  hl = legend('CO2','N2O','CH4','CFC11','CFC12','stemp*10'); set(hl,'fontsize',10);
