...
elseif abs(settings.dataset) >= 1 & abs(settings.dataset) <= 3
  ...
  elseif settings.descORasc == +1 & driver.i16daytimestep < 0 & settings.dataset == 2    
    ...
    driver.rateset.datafile  = [];
    if settings.ocb_set == 0  & driver.i16daytimestep < 0
      driver.rateset.datafile  = 'yayayayayayayaya'
      driver.rateset.datafile  = ['iType_2_convert_sergio_clearskygrid_obsonly_Q' num2str(driver.iQuantile,'%02d') '.mat'];           
      driver.rateset.datafile  = ['/home/sergio/MATLABCODE/oem_pkg_run_sergio_AuxJacs/TILES_TILES_TILES_MakeAvgCldProfs2002_2020/DATAObsStats_StartSept2002_CORRECT_LatLon/LatBin' num2str(driver.iLat,'%02d') '/trends_zonalavg_fits_quantiles_LatBin' num2str(driver.iLat,'%02d') '_timesetps_001_429_V1.mat'];
...
if driver.i16daytimestep < 0
  ...
  elseif settings.descORasc == +1
    ...
    driver.jac_latbin         = driver.iibin
    driver.jac_indexINSIDEbin = -1
...
[m_ts_jac0,nlays,qrenorm,freq2645]  = get_jac_fast(driver.jacobian.filename,(driver.iibin-1)*72 + 36,driver.iLat);
...
figure(7); clf; imagesc(m_ts_jac0(:,driver.jacobian.water_i)'); title('WV jac'); colorbar; colormap(jet)
figure(8); clf; imagesc(m_ts_jac0(:,driver.jacobian.temp_i)');  title('Tz jac'); colorbar; colormap(jet);
figure(9); clf; imagesc(m_ts_jac0(:,driver.jacobian.ozone_i)'); title('O3 jac'); colorbar; colormap(jet);
figure(10); plot(1:2645,driver.jacobian.colWV_values,1:2645,driver.jacobian.colTz_values,1:2645,driver.jacobian.colO3_values)
figure(11); plot(1:2645,driver.jacobian.scalar_values);
size(m_ts_jac0)
