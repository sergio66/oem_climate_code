...
%%%%%%%%%%%%%%%%%%%%%%%%%
if exist('iFixWV_NoFit','var') & (strcmp(driver.rateset.ocb_set,'obs') | strcmp(driver.rateset.ocb_set,'cal'))

  disp(' >>>>> need to account for cal_WV_rates by eg subbing in WV(z,t) from fits to cal or raw ERA anoms >>>>> ')
  xb(driver.jacobian.water_i) = cal_WV_rates;

  aux.FixWV_NoFit = cal_WV_rates;
  aux.orig_water_i = driver.jacobian.water_i;
  aux.orig_temp_i = driver.jacobian.temp_i;
  aux.orig_water_i = driver.jacobian.water_i;

  noWV = setdiff(1:length(xb),driver.jacobian.water_i);

  driver.jacobian.water_i =  [];

  m_ts_jac = m_ts_jac(:,noWV);
  aux.m_ts_jac    = m_ts_jac;

  xb = xb(noWV);
  driver.rateset.rates = driver.rateset.rates - spectra_due_to_WV_jac;   %% LARRABEE DOES NOT WANT THIS
  driver.qrenorm = driver.qrenorm(noWV);
end
