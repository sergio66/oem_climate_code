...
for ii = 1 : 1
  for jj = 1 : nlays
    junk = polyfit(1:365,wv(:,jj)',1); trendwv(ii,jj) = junk(1)*365/16;  %% junk is per 16 days, so in 1 year (365 days) we convert!
    junk = polyfit(1:365,tz(:,jj)',1); trendtz(ii,jj) = junk(1)*365/16;
    junk = polyfit(1:365,o3(:,jj)',1); trendo3(ii,jj) = junk(1)*365/16;

    %[B, err, stats]=Math_tsfit_lin_robust(x,y,n)
    B = Math_tsfit_lin_robust((1:365)*16,wv(:,jj),4); trendwv2(ii,jj) = B(2);
    B = Math_tsfit_lin_robust((1:365)*16,tz(:,jj),4); trendtz2(ii,jj) = B(2);
    B = Math_tsfit_lin_robust((1:365)*16,o3(:,jj),4); trendo32(ii,jj) = B(2);

    %[B, err, stats]=Math_tsfit_lin_robust(x,y,n)
    %B = Math_tsfit_lin_robust((1:365)*16,smooth(wv(:,jj),2*5*2),4); trendwv2(ii,jj) = B(2);
    %B = Math_tsfit_lin_robust((1:365)*16,smooth(tz(:,jj),2*5*2),4); trendtz2(ii,jj) = B(2);
    %B = Math_tsfit_lin_robust((1:365)*16,smooth(o3(:,jj),2*5*2),4); trendo32(ii,jj) = B(2);
