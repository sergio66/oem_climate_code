...
if exist('climcapsL3')
  iFig = 50;
  Tlevs = climcapsL3.Tlevs; lenTlevs = length(Tlevs);
  Qlevs = climcapsL3.Qlevs; lenQlevs = length(Qlevs);

  boo = zeros(72,64,lenQlevs); for ijunk = 1 : lenQlevs; boo(:,:,ijunk) = maskLFmatr'; end
  junk = climcapsL3.thestats64x72.waterrate.*boo; junk = reshape(junk,72*64,lenQlevs);  
  ...
  junk = climcapsL3.thestats64x72.waterratestd.*boo; junk = reshape(junk,72*64,lenQlevs);  
  ...
  boo = zeros(72,64,lenQlevs); for ijunk = 1 : lenQlevs; boo(:,:,ijunk) = maskLFmatr'; end
  junk = climcapsL3.thestats64x72.RHrate.*boo; junk = reshape(junk,72*64,lenQlevs);  
  ...
  junk = climcapsL3.thestats64x72.RHratestd.*boo; junk = reshape(junk,72*64,lenQlevs);  
  ...
  junk = climcapsL3.thestats64x72.ptemprate.*boo; junk = reshape(junk,72*64,lenTlevs);  
  ...
  junk = climcapsL3.thestats64x72.ptempratestd.*boo; junk = reshape(junk,72*64,lenTlevs);  
  ...
  [~,~,lenO3levs] = size(climcapsL3.thestats64x72.ozonerate);
  ...
  junk = climcapsL3.thestats64x72.ozonerate.*boo; junk = reshape(junk,72*64,lenO3levs);  
  ...
  junk = climcapsL3.thestats64x72.ozoneratestd.*boo; junk = reshape(junk,72*64,lenO3levs);  
...
if exist('merra2')
  ...
  junk = merra2.trend_gas_1; junk = reshape(junk,100,72,64); junk = junk.*boo; junk = reshape(junk,100,72*64);
  iFig = iFig + 1; figure(iFig);   subplot(121); hold on; semilogy(nanmean(nan_bottom100.*junk,2)',Qlevs,'linewidth',2);         hold off; ylim([1 1000]); set(gca,'ydir','reverse'); title('WV_{\mu} frac pert');
  junk = merra2.trend_gas_1_err; junk = reshape(junk,100,72,64); junk = junk.*boo; junk = reshape(junk,100,72*64);
                                   subplot(122); hold on; semilogy(1+nanmean(nan_bottom100.*junk,2)',Qlevs,'linewidth',2);     hold off; ylim([1 1000]); set(gca,'ydir','reverse'); title('WV_{\sigma} frac pert');

  junk = merra2.trend_RH; junk = reshape(junk,100,72,64); junk = junk.*boo; junk = reshape(junk,100,72*64);
  iFig = iFig + 1; figure(iFig);   subplot(121); hold on; semilogy(nanmean(nan_bottom100.*junk,2)',Qlevs,'linewidth',2);        hold off; ylim([1 1000]); set(gca,'ydir','reverse'); title('RH_{\mu} pert (%)');
  junk = merra2.trend_RH_err; junk = reshape(junk,100,72,64); junk = junk.*boo; junk = reshape(junk,100,72*64);
                                   subplot(122); hold on; semilogy(nanmean(nan_bottom100.*junk,2)',Qlevs,'linewidth',2);      hold off; ylim([1 1000]); set(gca,'ydir','reverse'); title('RH_{\sigma} pert (%)');

  junk = merra2.trend_ptemp; junk = reshape(junk,100,72,64); junk = junk.*boo; junk = reshape(junk,100,72*64);
  iFig = iFig + 1; figure(iFig);   subplot(121); hold on; semilogy(nanmean(nan_bottom100.*junk,2)',Tlevs,'linewidth',2);         hold off; ylim([1 1000]); set(gca,'ydir','reverse'); title('T_{\mu} pert');
  junk = merra2.trend_ptemp_err; junk = reshape(junk,100,72,64); junk = junk.*boo; junk = reshape(junk,100,72*64);
                                   subplot(122); hold on; semilogy(nanmean(nan_bottom100.*junk,2)',Tlevs,'linewidth',2);       hold off; ylim([1 1000]); set(gca,'ydir','reverse'); title('T_{\sigma} pert');

  junk = merra2.trend_gas_3; junk = reshape(junk,100,72,64); junk = junk.*boo; junk = reshape(junk,100,72*64);
  iFig = iFig + 1; figure(iFig);   subplot(121); hold on; semilogy(nanmean(nan_bottom100.*junk,2)',Tlevs,'linewidth',2);   hold off; ylim([0.01 1000]); set(gca,'ydir','reverse'); title('O3_{\mu} ppm pert');
  junk = merra2.trend_gas_3_err; junk = reshape(junk,100,72,64); junk = junk.*boo; junk = reshape(junk,100,72*64);
                                   subplot(122); hold on; semilogy(nanmean(nan_bottom100.*junk,2)',Tlevs,'linewidth',2); hold off; ylim([0.01 1000]); set(gca,'ydir','reverse'); title('O3_{\sigma} ppm pert');
end

iFig = 50;
iFig = iFig+1; figure(iFig); subplot(121); hold on; plotaxis2; hl = legend('UMBC','CLIMCAPSL3','AMIP6','MERRA2','location','best','fontsize',8); hold off; xlim([-1 +1]*2e-2)
                             subplot(122); hold on; plotaxis2; hl = legend('UMBC','CLIMCAPSL3','AMIP6','MERRA2','location','best','fontsize',8); hold off; xlim([0 0.01]+1)
iFig = iFig+1; figure(iFig); subplot(121); hold on; plotaxis2; hl = legend('UMBC','CLIMCAPSL3','AMIP6','MERRA2','location','best','fontsize',8); hold off; xlim([-1 +1]*0.25)
                             subplot(122); hold on; plotaxis2; hl = legend('UMBC','CLIMCAPSL3','AMIP6','MERRA2','location','best','fontsize',8); hold off; xlim([0 1]/2)
iFig = iFig+1; figure(iFig); subplot(121); hold on; plotaxis2; hl = legend('UMBC','CLIMCAPSL3','AMIP6','MERRA2','location','best','fontsize',8); hold off; xlim([-1 +1]*0.05) 
                             subplot(122); hold on; plotaxis2; hl = legend('UMBC','CLIMCAPSL3','AMIP6','MERRA2','location','best','fontsize',8); hold off; xlim([0 1]*0.1)
iFig = iFig+1; figure(iFig); subplot(121); hold on; plotaxis2; hl = legend('UMBC','CLIMCAPSL3','AMIP6','MERRA2','location','best','fontsize',8); hold off; xlim([-1 +1]*0.04)
                             subplot(122); hold on; plotaxis2; hl = legend('UMBC','CLIMCAPSL3','AMIP6','MERRA2','location','best','fontsize',8); hold off; xlim([0 1]*0.04)
...
if junkk == -1
  display(' ')
  display('in plot_profile_trends3_amip6.m --> make_profile_spectral_trends.m  we have iDoSpectralRates = -1 .. but you may have CLIMCAPSL3/MERRA2/AMIP6 spectral model trends from the call to ../FIND_NWP_MODEL_TRENDS/driver_get_the_model_trends')
  display('in plot_profile_trends3_amip6.m --> make_profile_spectral_trends.m  we have iDoSpectralRates = -1 .. but you may have CLIMCAPSL3/MERRA2/AMIP6 spectral model trends from the call to ../FIND_NWP_MODEL_TRENDS/driver_get_the_model_trends')
  display('in plot_profile_trends3_amip6.m --> make_profile_spectral_trends.m  we have iDoSpectralRates = -1 .. but you may have CLIMCAPSL3/MERRA2/AMIP6 spectral model trends from the call to ../FIND_NWP_MODEL_TRENDS/driver_get_the_model_trends')
  display(' ')
end
...
if junkk < 0
  ...
  if isfield(merra2,'merra2_spectral_rates')
    nwp_spectral_trends_amip6_merra2_climcapsl3_umbc.merra2_spectral_rates = merra2.merra2_spectral_rates;
  end
  if isfield(amip6,'amip6_spectral_rates')
    nwp_spectral_trends_amip6_merra2_climcapsl3_umbc.amip6_spectral_rates = amip6.amip6_spectral_rates;
  end
  if isfield(climcapsl3,'climcapsl3_spectral_rates')
    nwp_spectral_trends_climcapsl3_merra2_climcapsl3_umbc.climcapsl3_spectral_rates = climcapsl3.climcapsl3_spectral_rates;
  end
elseif junkk > 0
  iVersJac = input('Enter jac version (2019) = 2002/2019 ERAI or (2021) = 2002/2021 MERRA2 default : ');
  ...
  if ~exist('nwp_spectral_trends_amip6_merra2_climcapsl3_umbc')
    if exist('amip6')
      ...
      %% see SyntheticTimeSeries_MERRA2_CLIMCAPSL3_AMIP6/compare_sarta_kcarta_latbin32_trends.m
      for lll = 1 : 64
        %% see eg plot_check_WV_T_RH_AMIP6_geo_and_spectral_rates2.m
        dirout = '../FIND_NWP_MODEL_TRENDS/SimulateTimeSeries/MERRA2/';
        fsarta = [dirout '/reconstruct_merra2_spectra_geo_rlat' num2str(lll,'%02i') '.mat'];
        x = load(fsarta);
        ind = (1:72) + (lll-1)*72;
        sartaMERRA2trend(:,ind) = x.thesave.xtrendSpectral;
      end
      ...
      figure(1); plot(f,nanmean(sartaMERRA2trend'),f,nanmean(nwp_spectral_trends_amip6_merra2_climcapsl3_umbc.merra2_spectral_rates')); xlim([640 1640]); hl = legend('from SARTA trends','from jac x dX/dt','location','best');
      figure(1); lll = 1 : 4608; plot(f,sartaMERRA2trend(:,lll)-nwp_spectral_trends_amip6_merra2_climcapsl3_umbc.merra2_spectral_rates(:,lll));
      figure(1); plot(f,nanmean(sartaMERRA2trend'-nwp_spectral_trends_amip6_merra2_climcapsl3_umbc.merra2_spectral_rates'),f,nanstd(sartaMERRA2trend'-nwp_spectral_trends_amip6_merra2_climcapsl3_umbc.merra2_spectral_rates'),...
                      f,nanmean(sartaMERRA2trend'),'.-',f,nanmean(nwp_spectral_trends_amip6_merra2_climcapsl3_umbc.merra2_spectral_rates'))

%{
%% this is DA PLOT
plot(f,nanmean(rates(:,mask),2),'b',...
      f,nanmean(nwp_spectral_trends_amip6_merra2_climcapsl3_umbc.climcapsl3_spectral_rates(:,mask),2),'c',...
      f,nanmean(nwp_spectral_trends_amip6_merra2_climcapsl3_umbc.era_spectral_rates(:,mask),2),'g',...
      f,nanmean(nwp_spectral_trends_amip6_merra2_climcapsl3_umbc.merra2_spectral_rates(:,mask),2),'r',...
      f,nanmean(nwp_spectral_trends_amip6_merra2_climcapsl3_umbc.fits(:,mask),2),'m','linewidth',2)
plot(f,nanmean(rates(:,mask),2),'b',...
     f,nanmean(nwp_spectral_trends_amip6_merra2_climcapsl3_umbc.climcapsl3_spectral_rates(:,mask),2),'c',...
     f,nanmean(nwp_spectral_trends_amip6_merra2_climcapsl3_umbc.merra2_spectral_rates(:,mask),2),'r',...
     f,nanmean(nwp_spectral_trends_amip6_merra2_climcapsl3_umbc.amip6_spectral_rates(:,mask),2),'g',...
     f,nanmean(nwp_spectral_trends_amip6_merra2_climcapsl3_umbc.umbc_spectral_rates(:,mask),2),'m',...
     'linewidth',2)
  plotaxis2; hl = legend('AIRS Obs','AIRS L3','MERRA2','AMIP6','UMBC','location','best'); axis([640 1640 -0.1 0.05]); title('Spectral Rates');
plot(f,nanmean(rates(:,mask),2),...
     f,nanmean(nwp_spectral_trends_amip6_merra2_climcapsl3_umbc.climcapsl3_spectral_rates(:,mask),2),...
     f,nanmean(nwp_spectral_trends_amip6_merra2_climcapsl3_umbc.merra2_spectral_rates(:,mask),2),...
     f,nanmean(nwp_spectral_trends_amip6_merra2_climcapsl3_umbc.amip6_spectral_rates(:,mask),2),...
     'linewidth',0.5)
  plotaxis2; hl = legend('AIRS Obs','AIRS L3','MERRA2','AMIP6','location','best','fontsize',10); ylabel('dBT/dt K/yr'); xlabel('Wavenumber cm^{-1}'); axis([640 1640 -0.1 0.05]); %title('Spectral Rates');
...
%}
    ...
    addpath /home/sergio/MATLABCODE/CONVERT_GAS_UNITS
      ...
      [xall,x200,xtrop,xUTLS] = find_umbc_nwp_profile_trends_mmw_strat(h,p,pavg,nwp_spectral_trends_amip6_merra2_climcapsl3_umbc,resultsT,resultsWV,resultsO3,results(:,6),maskLFmatr,2);
  ...
  plot(f,nanmean(rates(:,mask),2),'k',f,nanmean(fits(:,mask),2),...
                                      f,nanmean(nwp_spectral_trends_amip6_merra2_climcapsl3_umbc.climcapsl3_spectral_rates(:,mask),2),...
                                      f,nanmean(nwp_spectral_trends_amip6_merra2_climcapsl3_umbc.amip6_spectral_rates(:,mask),2),...
                                      f,nanmean(nwp_spectral_trends_amip6_merra2_climcapsl3_umbc.merra2_spectral_rates(:,mask),2),'linewidth',2)
  plot(f,nanmean(rates(:,mask),2),'k',f,nanmean(nwp_spectral_trends_amip6_merra2_climcapsl3_umbc.umbc_spectral_rates(:,mask),2),...
                                      f,nanmean(nwp_spectral_trends_amip6_merra2_climcapsl3_umbc.climcapsl3_spectral_rates(:,mask),2),...
                                      f,nanmean(nwp_spectral_trends_amip6_merra2_climcapsl3_umbc.amip6_spectral_rates(:,mask),2),...
                                      f,nanmean(nwp_spectral_trends_amip6_merra2_climcapsl3_umbc.merra2_spectral_rates(:,mask),2),'linewidth',2)
  plotaxis2; hl = legend('AIRS Obs','UMBC','AIRS L3','AMIP6','MERRA2','location','best'); axis([640 1640 -0.1 0.05]); title('Spectral Rates');

  plot(f,nanmean(nwp_spectral_trends_amip6_merra2_climcapsl3_umbc.umbc_spectral_rates(:,mask),2),'x-',...
       f,nanmean(nwp_spectral_trends_amip6_merra2_climcapsl3_umbc.climcapsl3_spectral_rates(:,mask),2),...
       f,nanmean(nwp_spectral_trends_amip6_merra2_climcapsl3_umbc.amip6_spectral_rates(:,mask),2),...
       f,nanmean(nwp_spectral_trends_amip6_merra2_climcapsl3_umbc.merra2_spectral_rates(:,mask),2),f,nanmean(rates(:,mask),2),'linewidth',2)
  plotaxis2; hl = legend('UMBC','AIRS L3','AMIP6','MERRA2','AIRS Obs','location','best','fontsize',8); axis([640 1640 -0.1 0.05]); title('Spectral Rates');
end
