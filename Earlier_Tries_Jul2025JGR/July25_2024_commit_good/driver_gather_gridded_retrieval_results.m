...
elseif iOCBset == 1
  ...
  % end
  datafile  = ['/umbc/rs/pi_sergio/WorkDirDec2025/oem_climate_code/AIRS_gridded_STM_May2021_trendsonlyCLR/SyntheticTimeSeries_ERA5_AIRSL3_CMIP6/STS/NIGHTorAVG/ERA5/'  strY '/ERA5_spectraltrends_2002_09_2022_08.mat'];
  junk = load(datafile);
  ...
  i1419 = find(junk.h.vchan >= 1419,1);
  i1231 = find(junk.h.vchan >= 1231,1);
  i0900 = find(junk.h.vchan >= 0900,1);

  figure(6); clf; scatter_coast(XX,YY,50,data_trends.b_desc(1520,:)); colormap(usa2); caxis([-0.15 +0.15]); title('dBT1231/dt'); pause(0.1)
  aslmap(6,rlat65,rlon73,smoothn((reshape(data_trends.b_desc(i1231,:),72,64)'),1), [-90 +90],[-180 +180]); title('dBT1231/dt'); caxis([-1 +1]*0.15); colormap(llsmap5)
  aslmap(7,rlat65,rlon73,smoothn((reshape(data_trends.b_desc(i1419,:),72,64)'),1), [-90 +90],[-180 +180]); title('dBT1419/dt'); caxis([-1 +1]*0.15); colormap(llsmap5)  
end
...
if dataset < 30
  ...
  moonoise = load('/umbc/rs/pi_sergio/WorkDirDec2025/oem_climate_code/AIRS_gridded_STM_May2021_trendsonlyCLR/iType_19_iQAX_3_convert_sergio_clearskygrid_obsonly_Q03.mat','b_err_desc');  
elseif dataset == 30
...
figure(30); clf; waha = squeeze(nanmean(reshape(resultsWV,72,64,iNumLay),1)); waha = waha';       pcolor(rlat,pavg,waha);  shading interp; colorbar; set(gca,'ydir','reverse'); title('UMBC dWVfrac/dt'); colormap(llsmap5); caxis([-1 +1]*0.01)
  figure(29); set(gca,'yscale','log'); ylim([10 1000]);       figure(29); set(gca,'yscale','linear'); ylim([100 1000]);
