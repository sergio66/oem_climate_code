...
iLat  = driver.latlon.latbin;
iLon  = driver.latlon.lonbin;
iTime = driver.latlon.timestep;

latstr = ['lat_' num2str(iLat)];
lonstr = ['lat_' num2str(iLon)];
...
elseif settings.dataset == 1   %% DEFAULT
  ...
  if settings.descORasc == +1 & driver.i16daytimestep > 0
    ...
    if settings.ocb_set == 0  & driver.i16daytimestep > 0
      ...
      driver.rateset.datafile  = ['/home/strow/Work/Airs/Tiles/Data/era_grid_by_time/' latstr '/robs_fit_grid_lat_lon_' num2str(iLat) '_' num2str(iLon) '.mat'];
...
if driver.i16daytimestep > 0
  ...
  elseif iXJac == 2  
    ...
    driver.jac_latbin          = driver.latlon.latbin;
    %driver.jac_indexINSIDEbin = driver.latlon.lonbin;
    %driver.jac_indexINSIDEbin  = driver.iibin - (driver.jac_latbin-1)*72;
    driver.jac_indexINSIDEbin  = driver.latlon.lonbin + (driver.jac_latbin-1)*72;
    driver.jac_indexINSIDEbin  = driver.latlon.lonbin;

    [-987645 driver.latlon.latbin driver.latlon.lonbin driver.iibin driver.jac_latbin driver.jac_indexINSIDEbin +987654]
