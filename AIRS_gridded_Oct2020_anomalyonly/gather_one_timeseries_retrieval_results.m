...
iWarning = 0;
for ii = 1 : iTimeMax
  fname = ['OutputAnomaly_OBS/32/01/anomtest_timestep' num2str(ii) '.mat'];
  if exist(fname)
    iExist = +1;
    loader = ['load ' fname];
    eval(loader);
...
figure(1); plot(1:iTimeMax,results(:,1)); title('CO2');
figure(2); plot(1:iTimeMax,results(:,2)); title('N2O');
figure(3); plot(1:iTimeMax,results(:,3)); title('CH4');
figure(4); plot(1:iTimeMax,results(:,4)); title('Cld1');
figure(5); plot(1:iTimeMax,results(:,5)); title('Cld2');
figure(6); plot(1:iTimeMax,results(:,6)); title('ST');

junk = load('h2645structure.mat');
f           = junk.h.vchan;
figure(7);  plot(f,nanmean(rates'),'b',f,nanmean(rates'-fits'),'r'); grid
i730 = find(f >= 730,1);   figure(8);  plot(1:iTimeMax,rates(i730,:)-fits(i730,:)); title('BT730 bias')
i900 = find(f >= 900,1);   figure(9);  plot(1:iTimeMax,rates(i900,:)-fits(i900,:)); title('BT900 bias')
i1419 = find(f >= 1419,1); figure(10); plot(1:iTimeMax,rates(i1419,:)-fits(i1419,:)); title('BT1419 bias')
i667 = find(f >= 667,1);   figure(11); plot(1:iTimeMax,rates(i667,:)-fits(i667,:)); title('BT667 bias')
