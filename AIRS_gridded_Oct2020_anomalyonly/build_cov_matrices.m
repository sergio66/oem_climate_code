...
elseif driver.i16daytimestep < 0
  ...
  cov_set = [1.0  0.05/2*1000.0  0.05/2*1000.0  1/2       0.15/25*1000.0     0.15/25*1000.0   1/2      0.05/2*1000.0   0.05/0.75*1000.0     1/2        20*1E-3     20*1E-1  20*1E-1];  %**** gives small spectral bias, too high /WV rates, somewhat ok tracegas rates when kCARTA jacs used Oct 1, 2020

  %% *** >>>  WORKS FOR AIRS STM Oct 2020 <<< ****
...
if settings.set_tracegas == 1 & settings.co2lays == 1 & driver.i16daytimestep < 0
  ...
  %fmatd(1:5) = fmatd(1:5)*1e-1;    %% gives small spectral bias, lousy SARTA trace gas rates eg CO2=1.0, semi ok KCARTA trace gas rates  ****** WORKS_Oct1_2020_11pm_Showedstrow
  %fmatd(1:5) = fmatd(1:5)*1e-2;    %% gives small spectral bias, lousy SARTA trace gas rates eg CO2=1.0, semi ok KCARTA trace gas rates  ****** WORKS_Oct2_2020_11am_Showedstrow
  %fmatd(1:3) = fmatd(1:3)*1e-2;    %% gives small spectral bias, lousy SARTA trace gas rates eg CO2=1.0, semi ok KCARTA trace gas rates  ****** WORKS_Oct2_2020_11am_Showedstrow, WORKS_Oct2_2020_4pm

  %% *** >>>  WORKS FOR AIRS STM Oct 2020 <<< ****  
  fmatd(1:5) = fmatd(1:5)*1e-1;    %% gives small spectral bias, lousy SARTA trace gas rates eg CO2=1.0, semi ok KCARTA trace gas rates   *** >>>  WORKS FOR AIRS STM Oct 2020 <<< ****
  %% *** >>>  WORKS FOR AIRS STM Oct 2020 <<< ****

  %fmatd(1:5) = fmatd(1:5)*1e-2;    %% gives small spectral bias, lousy SARTA trace gas rates eg CO2=1.0, semi ok KCARTA trace gas rates 
