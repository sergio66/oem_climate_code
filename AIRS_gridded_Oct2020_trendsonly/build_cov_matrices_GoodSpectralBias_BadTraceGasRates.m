...
elseif driver.i16daytimestep < 0
  ...
  cov_set = [1.0  0.05/2*10000.0  0.05/2*10000.0  1/2       0.15/25*10000.0     0.15/25*10000.0   1/2      0.05/2*10000.0   0.05/0.75*10000.0     1/2        20*1E-1     20*1E-1  20*1E-1]; %%%%GREAT maybe loosen up a little
...
if settings.set_tracegas == 1 & settings.co2lays == 1 & driver.i16daytimestep < 0
  ...
  fmatd(1:3) = fmatd(1:3)*8e-7;    %% xb(1:3) (Co2/N2o/Ch4) so we should not change those values .. gives small spectral bias, lousy trace gas rates eg CO2=1.0  ******
