disp('WARNING : only using latbins 19-20 for ERA avg')
xlat_index = 19:20;
figure(6); clf
  subplot(121)
  shadedErrorBarY(nanmean(water(lat_index,:)),plays,nanstd(water(lat_index,:)),'bo-',1);
  hold on
  shadedErrorBarY(nanmean(waterrate(xlat_index,:)),plays,nanstd(waterrate(xlat_index,:)),'rx-',1);
  ...
  hold on
  shadedErrorBarY(nanmean(ptemprate(xlat_index,:)),plays,nanstd(ptemprate(xlat_index,:)),'rx-',1);
