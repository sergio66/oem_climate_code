...
fprintf(1,'\n AIRS L1C dofs + 0.2 K noise : \n')
Se = diag((airsNedT + 0.2).^2);
iInd = (1:97)+2*97; Sa = dT * ones(1,97); Sa = diag(Sa.^2);      %% 1 K error
  [nak_T_airs,ndofs_T_airsAVG] = generic_ak_dof(airs.v1c,Se,Sa,jac20.rKc(ichan,iInd),'T');
iInd = (1:97)+0*97; Sa = dWV * ones(1,97); Sa = diag(Sa.^2);   %% exp10(0.0792)-1 = 0.2 = 20% error
  [nak_WV_airs,ndofs_WV_airsAVG] = generic_ak_dof(airs.v1c,Se,Sa,jac20.rKc(ichan,iInd),'WV');
iInd = (1:97)+1*97; Sa = dO3 * ones(1,97); Sa = diag(Sa.^2);    %% exp10(0.175)-1 = 0.2 = 50% error
  [nak_O3_airs,ndofs_O3_airsAVG] = generic_ak_dof(airs.v1c,Se,Sa,jac20.rKc(ichan,iInd),'O3');

fprintf(1,'\n CRIS FSR dof + 0.2 K noise : \n')
Se = diag((crisFSR_NeDT + 0.2).^2);
iInd = (1:97)+2*97; Sa = dT * ones(1,97); Sa = diag(Sa.^2);      %% 1 K error
  [nak_T_crisFSR,ndofs_T_crisFSR_AVG] = generic_ak_dof(crishr2.freq,Se,Sa,jac20.hi_rcris_all(iYcrisFSR,iInd),'T');
iInd = (1:97)+0*97; Sa = dWV * ones(1,97); Sa = diag(Sa.^2);   %% exp10(0.0792)-1 = 0.2 = 20% error
  [nak_WV_crisFSR,ndofs_WV_crisFSR_AVG] = generic_ak_dof(crishr2.freq,Se,Sa,jac20.hi_rcris_all(iYcrisFSR,iInd),'WV');
iInd = (1:97)+1*97; Sa = dO3 * ones(1,97); Sa = diag(Sa.^2);    %% exp10(0.175)-1 = 0.2 = 50% error
  [nak_O3_crisFSR,ndofs_O3_crisFSR_AVG] = generic_ak_dof(crishr2.freq,Se,Sa,jac20.hi_rcris_all(iYcrisFSR,iInd),'O3');

fprintf(1,'\n CHIRP A dofs + 0.2 K noise : \n')
bad = find(isinf(chirpNedTA) | isnan(chirpNedTA));
good = find(isfinite(chirpNedTA));
if length(bad) > 0
  fprintf(1,'ohoh found %6i bad numbers for CHIRP NeDT! interpolating \n',length(bad));  
...
iInd = (1:97)+2*97; Sa = dT * ones(1,97); Sa = diag(Sa.^2);      %% 1 K error
  [nak_T_chirpA,ndofs_T_chirpA_AVG] = generic_ak_dof(chirp.wnum,Se,Sa,jac20.med_rcris_all(iYchirp,iInd),'T');
iInd = (1:97)+0*97; Sa = dWV * ones(1,97); Sa = diag(Sa.^2);   %% exp10(0.0792)-1 = 0.2 = 20% error
  [nak_WV_chirpA,ndofs_WV_chirpA_AVG] = generic_ak_dof(chirp.wnum,Se,Sa,jac20.med_rcris_all(iYchirp,iInd),'WV');
iInd = (1:97)+1*97; Sa = dO3 * ones(1,97); Sa = diag(Sa.^2);    %% exp10(0.175)-1 = 0.2 = 50% error
  [nak_O3_chirpA,ndofs_O3_chirpA_AVG] = generic_ak_dof(chirp.wnum,Se,Sa,jac20.med_rcris_all(iYchirp,iInd),'O3');

fprintf(1,'\n CHIRP C dofs + 0.2 K noise : \n')
bad = find(isinf(chirpNedTC) | isnan(chirpNedTC));
good = find(isfinite(chirpNedTC));
if length(bad) > 0
  fprintf(1,'ohoh found %6i bad numbers for CHIRP NeDT! interpolating \n',length(bad));  
...
iInd = (1:97)+2*97; Sa = dT * ones(1,97); Sa = diag(Sa.^2);      %% 1 K error
  [nak_T_chirpC,ndofs_T_chirpC_AVG] = generic_ak_dof(chirp.wnum,Se,Sa,jac20.med_rcris_all(iYchirp,iInd),'T');
iInd = (1:97)+0*97; Sa = dWV * ones(1,97); Sa = diag(Sa.^2);   %% exp10(0.0792)-1 = 0.2 = 20% error
  [nak_WV_chirpC,ndofs_WV_chirpC_AVG] = generic_ak_dof(chirp.wnum,Se,Sa,jac20.med_rcris_all(iYchirp,iInd),'WV');
iInd = (1:97)+1*97; Sa = dO3 * ones(1,97); Sa = diag(Sa.^2);    %% exp10(0.175)-1 = 0.2 = 50% error
  [nak_O3_chirpC,ndofs_O3_chirpC_AVG] = generic_ak_dof(chirp.wnum,Se,Sa,jac20.med_rcris_all(iYchirp,iInd),'O3');

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

figure(4); plot(ndofs_WV_airsAVG.cdofs,1:97,'b',ndofs_WV_crisFSR_AVG.cdofs,1:97,'g',ndofs_WV_chirpA_AVG.cdofs,1:97,'r',ndofs_WV_chirpC_AVG.cdofs,1:97,'m','linewidth',2);
  hl = legend('AIRS L1C','CRIS FSR','AIRS 2 CHIRP','CRIS 2 CHIRP','location','best','fontsize',10);
  title('WV DOF')
figure(5); plot(ndofs_O3_airsAVG.cdofs,1:97,'b',ndofs_O3_crisFSR_AVG.cdofs,1:97,'g',ndofs_O3_chirpA_AVG.cdofs,1:97,'r',ndofs_O3_chirpC_AVG.cdofs,1:97,'m','linewidth',2);
  hl = legend('AIRS L1C','CRIS FSR','AIRS 2 CHIRP','CRIS 2 CHIRP','location','best','fontsize',10);
  title('O3 DOF')
figure(6); plot(ndofs_T_airsAVG.cdofs,1:97,'b',ndofs_T_crisFSR_AVG.cdofs,1:97,'g',ndofs_T_chirpA_AVG.cdofs,1:97,'r',ndofs_T_chirpC_AVG.cdofs,1:97,'m','linewidth',2);
  hl = legend('AIRS L1C','CRIS FSR','AIRS 2 CHIRP','CRIS 2 CHIRP','location','best','fontsize',10);
  title('T DOF')

%{
save dofs_cris_airs_chirp.mat *dofs*
%}
