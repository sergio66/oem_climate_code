...
jacERA  = [a0.jacCO2 a0.jacN2O a0.jacCH4 zeros(size(a0.jacCH4)) zeros(size(a0.jacCH4)) a0.jacST fliplr(w0.jacWV) fliplr(a0.jacT) fliplr(a0.jacO3)];  jacERA  = jacERA(i2834to2645,:);
jacUMBC = [a1.jacCO2 a1.jacN2O a1.jacCH4 zeros(size(a1.jacCH4)) zeros(size(a1.jacCH4)) a1.jacST fliplr(w1.jacWV) fliplr(a1.jacT) fliplr(a1.jacO3)];  jacUMBC = jacUMBC(i2834to2645,:);
...
figure(1); subplot(221); ii = 1; plot(jac0.f,squeeze(jac0.M_TS_jac_all(iiBin,:,ii)),'b.-',jac0.f,jacERA(:,ii)*2.2/ppmvLay2(iiBin),'r')
figure(1); subplot(222); ii = 2; plot(jac0.f,squeeze(jac0.M_TS_jac_all(iiBin,:,ii)),'b.-',jac0.f,jacERA(:,ii)*1.0e-3/ppmvLay4(iiBin),'r')
figure(1); subplot(223); ii = 3; plot(jac0.f,squeeze(jac0.M_TS_jac_all(iiBin,:,ii)),'b.-',jac0.f,jacERA(:,ii)*5.0e-3/ppmvLay6(iiBin),'r')
figure(1); subplot(224); ii = 6; plot(jac0.f,squeeze(jac0.M_TS_jac_all(iiBin,:,ii)),'b.-',jac0.f,jacERA(:,ii)*0.1,'r')
