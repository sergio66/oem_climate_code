...
pnew0.stemp(2) = pnew0.stemp(2) + 1;  %% increasing ST

pnew0.stemp(3) = pnew0.stemp(3) + 0.2; %% changed ST to new state
pnew0.stemp(4) = pnew0.stemp(3) + 1;  %% increasing ST
...
figure(7); 
subplot(211); plot(hx.vchan,(tjac_t1-tjac_t0)/0.2)  %% dT = 1 so no need to worry about that
  title('true (d^2BT)/(dTs dTs)')
...
savex.stemp_stemp_true(:,ii) = tjac_t1-tjac_t0;
