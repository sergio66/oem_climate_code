...
for ii = 10 : 30
  ...
  %plot(smooth(1000*(bonk.all_bt_anom(:,i790)-bonk.all_bt_anom(:,i792)),180))
  xstrow_moo_obs(ii-10+1,:,:)  = bonk.all_bt_anom(1:5424,:);
  xstrow_moo_time(ii-10+1,:,:) = bonk.all_times(1:5424,:);
end
...
xanom1231 = nanmean(xstrow_moo_obs(:,:,i1231),1);
xanom790 = nanmean(xstrow_moo_obs(:,:,i790),1);
xanom792 = nanmean(xstrow_moo_obs(:,:,i792),1);
...
figure(1)
plot(2002.75+(1:5424)/365,smooth(xanom1231,180),'b',2002.75+(1:5785)/365,smooth(anom1231,180),'r');
  title('BT 1231 anomaly'); hl = legend('Sergio','Strow','location','best'); set(hl,'fontsize',10);

figure(2)
plot(2002.75+(1:5424)/365,smooth(xanom790-xanom792,180),'b',2002.75+(1:5785)/365,smooth(anom790-anom792,180),'r');
