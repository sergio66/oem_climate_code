...
%}
  ...
  ax = axis; ax(1) = min(okdates); ax(2) = max(okdates); ax(3) = 10; ax(4) = 1000;; grid; set(gca,'ydir','reverse'); title('raw ERA tropical Tz smoothed over 10 years')
  caxis([-1 +1]); colorbar
  colormap(llsmap4.llsmap4);

  figure(8); pcolor(okdates,playsN,smoothtz' - smooth_era_ptempanom'); shading interp; colorbar
  ax = axis; ax(1) = min(okdates); ax(2) = max(okdates); ax(3) = 10; ax(4) = 1000;; grid; set(gca,'ydir','reverse'); title('(tropical - raw ERA ) Tz smoothed over 10 years')
  caxis([-1 +1]); colorbar
  colormap(llsmap4.llsmap4);
...
figure(13); pcolor(okdates,latbins,tzanomsig400); shading interp; title('Tz unc anom at 400 mb'); caxis([0 0.1]); colorbar
