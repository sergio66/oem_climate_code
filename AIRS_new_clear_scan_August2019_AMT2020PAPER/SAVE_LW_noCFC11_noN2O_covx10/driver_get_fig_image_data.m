...
clf
plot(dT-2*delta,nmean2,'ro',dT-1*delta,nmean2_10,'ms',dT+0*delta,nmean3,'bx',dT+1*delta,nmean4,'g.-',dT+2*delta,nmean5,'co','linewidth',2)
hl = legend('reg+cov','reg+covx10','cov only, x1','cov only, x10','reg only, x1','location','best');
hold on; errorbar(dT-2*delta,nmean2,nstd2,'color','r'); 
...
junk = [sqrt(R2(2,1)) sqrt(R2_10(2,1)) sqrt(R3(2,1)) sqrt(R4(2,1)) sqrt(R5(2,1))];
fprintf(1,'the corr coeffs are %8.6f %8.6f %8.6f %8.6f %8.6f \n',junk);
