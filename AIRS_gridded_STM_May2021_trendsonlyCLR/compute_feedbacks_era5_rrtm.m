...
%era5_spectral_olr.olr0_ecRad = driver_run_ecRad_rtp_loop_over_profiles(h,px);               %%% <<<<<<<<<<<<<<<<<<<<<<<<<<<
...
px = p;
%px.gas_2 = px.gas_2*(1+2.2/400);
junk = era5.trend_gas_1(1:100,:); junk(isnan(junk)) = 1; 
px.gas_1(1:100,:) = px.gas_1(1:100,:) .* (1 + junk(1:100,:));
[~,pjunk] = subset_rtp_allcloudfields(h,px,[],[],112);
  junk = driver_rrtm_no_xsec_nocloud_twoslab_band17only_loop(h,pjunk,0);
...
era5_spectral_olr.feedback.planck_rrtm = era5_spectral_olr.planck_rrtm-era5_spectral_olr.olr0_rrtm; 
  era5_spectral_olr.feedback.planck_rrtm = -era5_spectral_olr.feedback.planck_rrtm./era5.trend_stemp;
era5_spectral_olr.feedback.lapse_rrtm = era5_spectral_olr.lapse_rrtm-era5_spectral_olr.planck_rrtm;
  era5_spectral_olr.feedback.lapse_rrtm = -era5_spectral_olr.feedback.lapse_rrtm./era5.trend_stemp;
era5_spectral_olr.feedback.wv_rrtm = era5_spectral_olr.wv_rrtm-era5_spectral_olr.olr0_rrtm;
  era5_spectral_olr.feedback.wv_rrtm = -era5_spectral_olr.feedback.wv_rrtm./era5.trend_stemp;
era5_spectral_olr.feedback.skt_rrtm = era5_spectral_olr.skt_rrtm-era5_spectral_olr.olr0_rrtm;
  era5_spectral_olr.feedback.skt_rrtm = -era5_spectral_olr.feedback.skt_rrtm./era5.trend_stemp;

figure(75); scatter_coast(p.rlon,p.rlat,50,era5_spectral_olr.feedback.planck_rrtm); caxis([-4 0]*1.5);  colormap(jet);  title('ERA5 \lambda_{Planck}')
figure(76); scatter_coast(p.rlon,p.rlat,50,era5_spectral_olr.feedback.lapse_rrtm);  caxis([-5 +5]*2); colormap(usa2); title('ERA5 \lambda_{Lapse}')
figure(77); scatter_coast(p.rlon,p.rlat,50,era5_spectral_olr.feedback.wv_rrtm);     caxis([-2 +2]*2); colormap(usa2); title('ERA5 \lambda_{WV}')
figure(78); scatter_coast(p.rlon,p.rlat,50,era5_spectral_olr.feedback.skt_rrtm);    caxis([-2 0]*1);  colormap(jet);  title('ERA5 \lambda_{Skt}')

wonk = era5_spectral_olr.feedback.planck_rrtm; wonk(wonk < -10) = NaN; wonk(wonk > 0) = NaN; 
...
wonk = era5_spectral_olr.feedback.lapse_rrtm; wonk(wonk < -5) = NaN; wonk(wonk > +5) = NaN; 
...
wonk = era5_spectral_olr.feedback.wv_rrtm; wonk(wonk < -5) = NaN; wonk(wonk > +5) = NaN; 
...
wonk = era5_spectral_olr.feedback.skt_rrtm; wonk(wonk < -3) = NaN; wonk(wonk > +3) = NaN; 
...
bad = find(abs(era5.trend_stemp) < 1e-4);
junklat = reshape(p.rlat,72,64); junklat = mean(junklat,1);
figure(79); clf
  junk = era5_spectral_olr.feedback.planck_rrtm; junk(bad) = NaN; junk = reshape(junk,72,64);  junk = nanmean(junk,1); junk = smooth(junk,5); plot(junklat,junk,'b','linewidth',2); hold on
  junk = era5_spectral_olr.feedback.lapse_rrtm; junk(bad) = NaN; junk = reshape(junk,72,64);   junk = nanmean(junk,1); junk = smooth(junk,5); plot(junklat,junk,'g','linewidth',2); hold on
  junk = era5_spectral_olr.feedback.wv_rrtm; junk(bad) = NaN; junk = reshape(junk,72,64);      junk = nanmean(junk,1); junk = smooth(junk,5); plot(junklat,junk,'r','linewidth',2); hold on
  junk = era5_spectral_olr.feedback.skt_rrtm; junk(bad) = NaN; junk = reshape(junk,72,64);     junk = nanmean(junk,1); junk = smooth(junk,5); plot(junklat,junk,'k','linewidth',2); hold off
ylim([-10 +10]/2); plotaxis2; title('ERA5 \lambda'); xlabel('Latitude'); hl = legend('Planck','Lapse','WV','SKT','location','best','fontsize',8);

junk = era5_spectral_olr.feedback.planck_rrtm; junk(bad) = NaN;
