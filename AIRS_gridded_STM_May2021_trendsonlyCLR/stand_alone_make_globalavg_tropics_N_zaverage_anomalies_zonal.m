...
if ~exist('btavg_newlat1')    %% tropical average
  iCnt = 1;
    clear junk
    fprintf(1,'making TROPICAL avg %2i of %2i \n',iCnt, length(newLatGrid)-1);
    usethese{iCnt+1} = find(rlat >= -90 & rlat < +90);
    usethese{iCnt+1} = find(rlat >= newLatGrid(1) & rlat < newLatGrid(end));
    usethese{iCnt+1} = find(rlat >= -30 & rlat < +30);
    xusethese = usethese{iCnt+1};
    junkcoslat = coslat(xusethese,:);
    for ii = 1 : length(ind)
      moo = squeeze(btanomX(:,ii,:));
...
if iSave > 0
  if iDorA == +1
    ...
    hh = hhD; 
    rtime = rtimeD;
    fout = ['anomalyD_zonalavg_globalavg_and_tropics_and_' num2str(length(newLatGrid)-1) '_averages_timeseries_Q' num2str(iQuant,'%02d') '_numyears_' num2str(iNumYears,'%6.2f') '_iNumAnomTimeSteps_' num2str(iNumAnomTimeSteps) ];
  else
    yy = yyA; 
    mm = mmA; 
    dd = ddA; 
    hh = hhA; 
    rtime = rtimeA;
    fout = ['anomalyA_zonalavg_globalavg_and_tropics_and_' num2str(length(newLatGrid)-1) '_averages_timeseries_Q' num2str(iQuant,'%02d') '_numyears_' num2str(iNumYears,'%6.2f') '_iNumAnomTimeSteps_' num2str(iNumAnomTimeSteps) ];
  end
  if iQAX == 3
  ...
  comment = 'see stand_alone_make_globalavg_tropics_N_zaverage_anomalies_zonal.m';
...
a1 = load('anomalyD_zonalavg_globalavg_and_tropics_and_28_averages_timeseries_Q03_numyears_22_iNumAnomTimeSteps_500.mat');

ind0 = 1:500;     ind1 = 1:500;      sum(sum(a0.btavgAnomFinal(:,ind0)-a1.btavgAnomFinal(:,ind1)))   %% global
ind0 = 501:14500; ind1 = ind0 + 500; sum(sum(a0.btavgAnomFinal(:,ind0)-a1.btavgAnomFinal(:,ind1)))   %% the latbins; new one has tropics bewteen 501:1000
%}
