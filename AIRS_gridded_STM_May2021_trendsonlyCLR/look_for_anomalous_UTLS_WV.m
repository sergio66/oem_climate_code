...
plot(wsarta,g1jac(:,[i010mb i050mb]));
...
utlswv = squeeze(nanmean(reshape(utlswv .* coslat,72,64,iNumAnomTimeSteps),1));
...
hold off
xlabel('Wavenumber cm-1'); ylabel('BT mean over 4608 profiles')
disp('ret to continue'); pause
...
bt_Tz1 = squeeze(btanomX(:,iaTz1,:));                                      bt_Tz1 = squeeze(nanmean(reshape(bt_Tz1.*coslat,72,64,iNumAnomTimeSteps),1)); 
bt_Tz2 = squeeze(btanomX(:,iaTz2,:)); bt_Tz2 = squeeze(nanmean(bt_Tz2,2)); bt_Tz2 = squeeze(nanmean(reshape(bt_Tz2.*coslat,72,64,iNumAnomTimeSteps),1)); 
bt_Tz3 = squeeze(btanomX(:,iaTz3,:)); bt_Tz3 = squeeze(nanmean(bt_Tz3,2)); bt_Tz3 = squeeze(nanmean(reshape(bt_Tz3.*coslat,72,64,iNumAnomTimeSteps),1)); 
bt_Tz4 = squeeze(btanomX(:,iaTz4,:)); bt_Tz4 = squeeze(nanmean(bt_Tz4,2)); bt_Tz4 = squeeze(nanmean(reshape(bt_Tz4.*coslat,72,64,iNumAnomTimeSteps),1)); 
...
%% note how I have ordered it as 2,1,3,4 to have [28.2468   25.1267   21.9208   20.8077] km
...
line([2022+15/30/12 2022+15/30/12],[15 30],'color','k','linewidth',2)
...
bt_WV2 = squeeze(btanomX(:,iaWV2,:)); bt_WV2 = squeeze(nanmean(bt_WV2,2)); bt_WV2 = squeeze(nanmean(reshape(bt_WV2.*coslat,72,64,iNumAnomTimeSteps),1)); 
bt_WV1 = squeeze(btanomX(:,iaWV1,:)); bt_WV1 = squeeze(nanmean(bt_WV1,2)); bt_WV1 = squeeze(nanmean(reshape(bt_WV1.*coslat,72,64,iNumAnomTimeSteps),1)); 
...
bt_Q = (bt_WV2 - nanmean(nanmean(tzjac(iaWV2,i050mb),1),1) * bt_Tz2)/nanmean(nanmean(g1jac(iaWV2,i050mb),1),1)/1e4;
