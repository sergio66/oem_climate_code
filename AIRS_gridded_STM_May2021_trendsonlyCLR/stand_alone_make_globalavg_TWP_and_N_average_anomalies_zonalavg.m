...
stand_alone_make_globalavg_tropics_N_zaverage_anomalies_zonal

iTWP = 1; %% file saved into anomalyD_zonalavg_globalavg_and_TWPlat35lon66_and_28_averages_timeseries_Q03_numyears_22_iNumAnomTimeSteps_500.mat
iTWP = 0; %% file saved into anomalyD_zonalavg_globalavg_and_TWPlat35avg_and_28_averages_timeseries_Q03_numyears_22_iNumAnomTimeSteps_500.mat
mooTWP0 = load('/asl/s1/sergio/JUNK/anomaly_iQAX_3_zonalavg_ALL_Q03_numyears_22.00_iNumAnomTimeSteps_500_D.mat'); %% see put_together_QuantileChoose_anomalies_plots_AND_save_D.m -- this puts in an AVERAGE
mooTWP1 = load('btanomTWP_35_66_Q03_numyears_22_iNumAnomTimeSteps_500.mat');   %% this puts in lat35,lon66
load /asl/s1/sergio/JUNK/anomaly_iQAX_3_zonalavg_ALL_Q03_numyears_22.00_iNumAnomTimeSteps_500_D.mat
iDorA = +1; iQuant = 3;  %% Q0.90 for paper, iQAX = 3
stand_alone_make_globalavg_TWP_and_N_average_anomalies_zonalavg
...
if iDorA > 0
  ...
  disp('standa_lone_make_globalavg_and_N_average_anomalies_zonalavg.m using btanomD')
else
  if ~exist('daysSince2002A')
...
iTWP = -1;
error('what the moo')
if ~exist('btavg_newlat1')    %% tropical Lat 35, Lon 36
  iCnt = 1;
  clear junk
  woo = (35-1)*72 + 66;
    usethese{iCnt+1} = 35;
    xusethese = usethese{iCnt+1};
    fprintf(1,'Lat 25, lon 36 TWP avg %2i of %2i need to use %3i in average \n',iCnt, length(newLatGrid)-1, length(xusethese));        
    junkcoslat = coslat(xusethese,:);
    for ii = 1 : length(ind)
      moo = squeeze(btanomX(:,ii,:));
      moo = moo(35,:);
    ...
    if iTWP == 1
      fprintf(1,'making Lat 35, Lon 36 TWP avg, DIRECT SUBBING OF THE ACTUAL TILE YAY %2i of %2i \n',iCnt, length(newLatGrid)-1);
      junk = mooTWP1.btanomTWP_35_66D;
    end
    str = ['btavg_newlat' num2str(iCnt) ' = junk;'];
    eval(str);
end
  
if ~exist('btavg_newlat2')
  for iCnt = 1 : length(newLatGrid)-1
    clear junk
    usethese{iCnt+2} = find(rlat >= newLatGrid(iCnt) & rlat < newLatGrid(iCnt+1));
    xusethese = usethese{iCnt+2};
    fprintf(1,'making FATBIN avg %2i of %2i need to use %3i in average \n',iCnt, length(newLatGrid)-1,length(xusethese));
    junkcoslat = coslat(xusethese,:);
    for ii = 1 : length(ind)
      moo = squeeze(btanomX(:,ii,:));
      moo = moo(xusethese,:);
    ...
    str = ['btavg_newlat' num2str(iCnt+1) ' = junk;'];
    eval(str);
  end
  ...
  for ii = 0 : length(newLatGrid)-1+1
  ...
  for iCnt = 1 : length(newLatGrid)-1
    str = ['junk = btavg_newlat' num2str(iCnt+1) ';'];
    eval(str);
...
if iSave > 0
  iQAX = 3 %% MAKE SURE THIS IS CORRECT ?????
  ...
  if iDorA == +1
    ...
    hh = hhD; 
    rtime = rtimeD;
    fout = ['anomalyD_zonalavg_globalavg_and_TWPlat35_and_' num2str(length(newLatGrid)-1) '_averages_timeseries_Q' num2str(iQuant,'%02d') '_numyears_' num2str(iNumYears,'%6.2f') '_iNumAnomTimeSteps_' num2str(iNumAnomTimeSteps) ];
  else
    yy = yyA; 
    mm = mmA; 
    dd = ddA; 
    hh = hhA; 
    rtime = rtimeA;
    fout = ['anomalyA_zonalavg_globalavg_and_TWPlat35_and_' num2str(length(newLatGrid)-1) '_averages_timeseries_Q' num2str(iQuant,'%02d') '_numyears_' num2str(iNumYears,'%6.2f') '_iNumAnomTimeSteps_' num2str(iNumAnomTimeSteps) ];
  end
  if iQAX == 3
  ...
  comment = 'see stand_alone_make_globalavg_and_TWP_and_N_average_anomalies_zonalavg.m';
...
%{
clear all
a0 = load('anomalyD_zonalavg_globalavg_and_28_averages_timeseries_Q03_numyears_22_iNumAnomTimeSteps_500.mat');
a0 = load('anomalyD_zonalavg_globalavg_and_tropics_and_28_averages_timeseries_Q03_numyears_22_iNumAnomTimeSteps_500.mat');
a1 = load('anomalyD_zonalavg_globalavg_and_TWPlat35_and_28_averages_timeseries_Q03_numyears_22_iNumAnomTimeSteps_500.mat');

ind0 = 001:500;    ind1 = ind0;      sum(sum(a0.btavgAnomFinal(:,ind0)-a1.btavgAnomFinal(:,ind1)))   %% global, should be same
ind0 = 501:1000;   ind1 = ind0;      sum(sum(a0.btavgAnomFinal(:,ind0)-a1.btavgAnomFinal(:,ind1)))   %% tropical vs TWPlat35
ind0 = 1101:15000; ind1 = ind0;      sum(sum(a0.btavgAnomFinal(:,ind0)-a1.btavgAnomFinal(:,ind1)))   %% the latbins; new one has tropics bewteen 501:1000
%}
