...
figure(6); plot(rlon,thesave.st_trend,rlon,thesave.bt1231_trend); title('dST/dt and dBT1231/dt');
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for iii = 1 : 2645
  data = tcalcavg(iii,:);
  junk = polyfit(dayOFtime/365,tcalcavg(iii,:),1);
  thesave.trend(iii) = junk(1);
  constr(iii) = junk(2);

  junk = Math_tsfit_lin_robust(dayOFtime,tcalcavg(iii,:),4);
  thesave.xtrend(iii) = junk(2);
  xconstr(iii) = junk(1);
end

figure(2); plot(fchanx,thesave.trend); grid;  axis([640 1640 -0.1 +0.05]); plotaxis2; title('dBT/dt from CMIP6 or ERA5')
...
figure(2); plot(fchanx,thesave.trend,fchanx,thesave.xtrend,fchanx,nanmean(thesave.trendSpectral')); grid;
...
figure(3); pcolor(rlon,plevsnwp,thesave.t2d_trendnwp); shading interp; colorbar; colormap(llsmap5); caxis([-0.25 +0.25]); 
...
figure(4); pcolor(rlon,plevsx,thesave.t2d_trend(1:97,:)); shading interp; colorbar; colormap(llsmap5); caxis([-0.25 +0.25]); 
...
figure(3); pcolor(rlon,plevsnwp,thesave.rh2d_trendnwp); shading interp; colorbar; colormap(llsmap5); caxis([-0.25 +0.25]); 
...
figure(4); pcolor(rlon,plevsx,thesave.rh2d_trend(1:97,:)); shading interp; colorbar; colormap(llsmap5); caxis([-0.25 +0.25]); 
...
figure(5); subplot(122); semilogy(thesave.rh_trend(1:97),plevsx,'r','linewidth',2); ylim([100 1000]); set(gca,'ydir','reverse'); plotaxis2;
...
figure(5); subplot(121); semilogy(thesave.t_trend(1:97),plevsx,'r','linewidth',2); ylim([10 1000]); set(gca,'ydir','reverse');  plotaxis2;
