...
figure(2); plot(fchanx,trend); grid;  axis([640 1640 -0.1 +0.05]); plotaxis2; title('dBT/dt from ERA5')
figure(2); plot(fchanx,trend,fchanx,xtrend); grid;  axis([640 1640 -0.1 +0.05]); plotaxis2; title('dBT/dt from ERA5')
...
t = p64.ptemp; t = reshape(t,37,64,numtimesteps); 
for jj = 1 : 37
  for ll = 1 : 64
...
figure(3); pcolor(rlatx,plevsnwp,t2d_trendnwp); shading interp; colorbar; colormap(llsmap5); caxis([-0.25 +0.25]); 
  set(gca,'ydir','reverse'); set(gca,'yscale','log'); ylim([10 1000]); title('T rates straight from ERA5 zonal levels')
...
rh = p64.rh; rh = reshape(rh,37,64,numtimesteps); 
for jj = 1 : 37
  for ll = 1 : 64
...
figure(3); pcolor(rlatx,plevsnwp,rh2d_trendnwp); shading interp; colorbar; colormap(llsmap5); caxis([-0.25 +0.25]); 
  set(gca,'ydir','reverse'); set(gca,'yscale','log'); ylim([100 1000]); title('RH rates straight from ERA5 zonal levels')
...
saver = ['save reconstruct_era5_spectra_geo.mat fchanx trend* xtrend* rlatx '];
saver = [saver ' plevsnwp t2d_trendnwp rh2d_trendnwp '];
saver = [saver ' plevsx t2d_trend rh2d_trend rh_trend t_trend '];
saver = [saver ' zonalrlat zonalplays zonalRHERA5rate zonalTERA5rate st_trend bt1231_trend '];
eval(saver)
