...
wagaboo = ones(size(quickgas_3rate));

raReconstructWithUnc = raReconstructWithUnc + jST'*(quickstemprate+errstemprate);
bah = ones(2645,1) * (exp(quickgas_1rate(ixN)'+errgas_1rate(ixN)')-1); raReconstructWithUnc = raReconstructWithUnc + sum(bah.*jWV(:,ixN),2)';
bah = ones(2645,1) * (exp(quickgas_3rate(ixN)'+errgas_3rate(ixN)')-1); raReconstructWithUnc = raReconstructWithUnc + sum(bah.*jO3(:,ixN),2)';
...
%wagaboo = ones(size(quickgas_3rate)); noo = find(p72avg.plevs <= 100); wagaboo(noo) = p72avg.plevs(noo)/100;
%wagaboo = ones(size(quickgas_3rate)); noo = find(p72avg.plevs <= 100); wagaboo(noo) = p72avg.plevs(noo)*1.0005;
%wagaboo = ones(size(quickgas_3rate)); noo = find(p72avg.plevs <= 100); wagaboo(noo) = 0.00;

wagaboo = ones(size(quickgas_3rate)); noo = find(p72avg.plevs <= 400); wagaboo(noo) = +5.0;  %%% WORKS PRETTY WELL for entire year all seasons
wagaboo = ones(size(quickgas_3rate)); noo = find(p72avg.plevs <= 400); wagaboo(noo) = +1.5;  %%% WORKS PRETTY WELL for SUMMER JJA
wagaboo = ones(size(quickgas_3rate));

figure(2); plot(errgas_1rate(ixN),ixN,'k--',quickgas_1rate(ixN)',ixN,'bo-',wagaboo(ixN)'.*quickgas_1rate(ixN)',ixN,'r'); 
  set(gca,'ydir','reverse')

raReconstruct = raReconstruct + jST'*quickstemprate;
bah = ones(2645,1) * (exp(wagaboo(ixN)'.*quickgas_1rate(ixN)')-1); raReconstruct = raReconstruct + sum(bah.*jWV(:,ixN),2)';
