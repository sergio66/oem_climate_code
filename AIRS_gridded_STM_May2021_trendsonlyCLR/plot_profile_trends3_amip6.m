...
if exist('amip6')
  ...
  junk = amip6.trend_gas_1; junk = reshape(junk,100,72,64); junk = junk.*boo; junk = reshape(junk,100,72*64);
  ...
  junk = amip6.trend_gas_1_err; junk = reshape(junk,100,72,64); junk = junk.*boo; junk = reshape(junk,100,72*64);
  ...
  junk = amip6.trend_RH; junk = reshape(junk,100,72,64); junk = junk.*boo; junk = reshape(junk,100,72*64);
  ...
  junk = amip6.trend_RH_err; junk = reshape(junk,100,72,64); junk = junk.*boo; junk = reshape(junk,100,72*64);
  ...
  junk = amip6.trend_ptemp; junk = reshape(junk,100,72,64); junk = junk.*boo; junk = reshape(junk,100,72*64);
  ...
  junk = amip6.trend_ptemp_err; junk = reshape(junk,100,72,64); junk = junk.*boo; junk = reshape(junk,100,72*64);
  ...
  junk = amip6.trend_gas_3; junk = reshape(junk,100,72,64); junk = junk.*boo; junk = reshape(junk,100,72*64);
  ...
  junk = amip6.trend_gas_3_err; junk = reshape(junk,100,72,64); junk = junk.*boo; junk = reshape(junk,100,72*64);
...
iFig = 50;
iFig = iFig+1; figure(iFig); subplot(121); hold on; plotaxis2; hl = legend('UMBC','AIRSL3','AMIP6','ERA5','location','best','fontsize',8); hold off; xlim([-1 +1]*2e-2)
                             subplot(122); hold on; plotaxis2; hl = legend('UMBC','AIRSL3','AMIP6','ERA5','location','best','fontsize',8); hold off; xlim([0 0.01]+1)
iFig = iFig+1; figure(iFig); subplot(121); hold on; plotaxis2; hl = legend('UMBC','AIRSL3','AMIP6','ERA5','location','best','fontsize',8); hold off; xlim([-1 +1]*0.25)
                             subplot(122); hold on; plotaxis2; hl = legend('UMBC','AIRSL3','AMIP6','ERA5','location','best','fontsize',8); hold off; xlim([0 1]/2)
iFig = iFig+1; figure(iFig); subplot(121); hold on; plotaxis2; hl = legend('UMBC','AIRSL3','AMIP6','ERA5','location','best','fontsize',8); hold off; xlim([-1 +1]*0.05) 
                             subplot(122); hold on; plotaxis2; hl = legend('UMBC','AIRSL3','AMIP6','ERA5','location','best','fontsize',8); hold off; xlim([0 1]*0.1)
iFig = iFig+1; figure(iFig); subplot(121); hold on; plotaxis2; hl = legend('UMBC','AIRSL3','AMIP6','ERA5','location','best','fontsize',8); hold off; xlim([-1 +1]*0.04)
...
if junk > 0
  ...
  if ~exist('nwp_spectral_trends_amip6_era5_airsL3_umbc')
    if exist('amip6')
      ...
      nwp_spectral_trends_amip6_era5_airsL3_umbc = make_profile_spectral_trends(amip6,era5,airsL3,results,resultsWV,resultsT,resultsO3,fits,rates,pavg,plays,f,2,iVersJac);

      figure(1); plot(f,nanmean(sartaERA5trend'),f,nanmean(nwp_spectral_trends_amip6_era5_airsL3_umbc.era5_spectral_rates')); xlim([640 1640]); hl = legend('from SARTA trends','from jac x dX/dt','location','best');
      figure(1); lll = 1 : 4608; plot(f,sartaERA5trend(:,lll)-nwp_spectral_trends_amip6_era5_airsL3_umbc.era5_spectral_rates(:,lll));
      figure(1); plot(f,nanmean(sartaERA5trend'-nwp_spectral_trends_amip6_era5_airsL3_umbc.era5_spectral_rates'),f,nanstd(sartaERA5trend'-nwp_spectral_trends_amip6_era5_airsL3_umbc.era5_spectral_rates'),...
                      f,nanmean(sartaERA5trend'),'.-',f,nanmean(nwp_spectral_trends_amip6_era5_airsL3_umbc.era5_spectral_rates'))

%{
%% this is DA PLOT
plot(f,nanmean(rates(:,mask),2),'b',...
      f,nanmean(nwp_spectral_trends_amip6_era5_airsL3_umbc.airsL3_spectral_rates(:,mask),2),'c',...
      f,nanmean(nwp_spectral_trends_amip6_era5_airsL3_umbc.era_spectral_rates(:,mask),2),'g',...
      f,nanmean(nwp_spectral_trends_amip6_era5_airsL3_umbc.era5_spectral_rates(:,mask),2),'r',...
      f,nanmean(nwp_spectral_trends_amip6_era5_airsL3_umbc.fits(:,mask),2),'m','linewidth',2)
plot(f,nanmean(rates(:,mask),2),'b',...
     f,nanmean(nwp_spectral_trends_amip6_era5_airsL3_umbc.airsL3_spectral_rates(:,mask),2),'c',...
     f,nanmean(nwp_spectral_trends_amip6_era5_airsL3_umbc.era5_spectral_rates(:,mask),2),'r',...
     f,nanmean(nwp_spectral_trends_amip6_era5_airsL3_umbc.amip6_spectral_rates(:,mask),2),'g',...
     f,nanmean(nwp_spectral_trends_amip6_era5_airsL3_umbc.umbc_spectral_rates(:,mask),2),'m',...
     'linewidth',2)
  plotaxis2; hl = legend('AIRS Obs','AIRS L3','ERA5','AMIP6','UMBC','location','best'); axis([640 1640 -0.1 0.05]); title('Spectral Rates');
plot(f,nanmean(rates(:,mask),2),...
     f,nanmean(nwp_spectral_trends_amip6_era5_airsL3_umbc.airsL3_spectral_rates(:,mask),2),...
     f,nanmean(nwp_spectral_trends_amip6_era5_airsL3_umbc.era5_spectral_rates(:,mask),2),...
     f,nanmean(nwp_spectral_trends_amip6_era5_airsL3_umbc.amip6_spectral_rates(:,mask),2),...
     'linewidth',0.5)
  plotaxis2; hl = legend('AIRS Obs','AIRS L3','ERA5','AMIP6','location','best','fontsize',10); ylabel('dBT/dt K/yr'); xlabel('Wavenumber cm^{-1}'); axis([640 1640 -0.1 0.05]); %title('Spectral Rates');
...
%}
    ...
    addpath /home/sergio/MATLABCODE/CONVERT_GAS_UNITS
      ...
      [xall,x200,xtrop,xUTLS] = find_umbc_nwp_profile_trends_mmw_strat(h,p,pavg,nwp_spectral_trends_amip6_era5_airsL3_umbc,resultsT,resultsWV,resultsO3,results(:,6),maskLFmatr,2);
  ...
  plot(f,nanmean(rates(:,mask),2),'k',f,nanmean(fits(:,mask),2),...
                                      f,nanmean(nwp_spectral_trends_amip6_era5_airsL3_umbc.airsL3_spectral_rates(:,mask),2),...
                                      f,nanmean(nwp_spectral_trends_amip6_era5_airsL3_umbc.amip6_spectral_rates(:,mask),2),...
                                      f,nanmean(nwp_spectral_trends_amip6_era5_airsL3_umbc.era5_spectral_rates(:,mask),2),'linewidth',2)
  plot(f,nanmean(rates(:,mask),2),'k',f,nanmean(nwp_spectral_trends_amip6_era5_airsL3_umbc.umbc_spectral_rates(:,mask),2),...
                                      f,nanmean(nwp_spectral_trends_amip6_era5_airsL3_umbc.airsL3_spectral_rates(:,mask),2),...
                                      f,nanmean(nwp_spectral_trends_amip6_era5_airsL3_umbc.amip6_spectral_rates(:,mask),2),...
                                      f,nanmean(nwp_spectral_trends_amip6_era5_airsL3_umbc.era5_spectral_rates(:,mask),2),'linewidth',2)
  plotaxis2; hl = legend('AIRS Obs','UMBC','AIRS L3','AMIP6','ERA5','location','best'); axis([640 1640 -0.1 0.05]); title('Spectral Rates');

  plot(f,nanmean(nwp_spectral_trends_amip6_era5_airsL3_umbc.umbc_spectral_rates(:,mask),2),'x-',...
       f,nanmean(nwp_spectral_trends_amip6_era5_airsL3_umbc.airsL3_spectral_rates(:,mask),2),...
       f,nanmean(nwp_spectral_trends_amip6_era5_airsL3_umbc.amip6_spectral_rates(:,mask),2),...
       f,nanmean(nwp_spectral_trends_amip6_era5_airsL3_umbc.era5_spectral_rates(:,mask),2),f,nanmean(rates(:,mask),2),'linewidth',2)
  plotaxis2; hl = legend('UMBC','AIRS L3','AMIP6','ERA5','AIRS Obs','location','best','fontsize',8); axis([640 1640 -0.1 0.05]); title('Spectral Rates');
end
...
if ~exist('compute_deltaRH')
  ...
  disp(' ')
  disp('computing AMIP6   spectral closure'); 
    compute_deltaRH.amip6 = const_rh_4608(nwp_spectral_trends_amip6_era5_airsL3_umbc.amip6_100_layertrends.stemp,nwp_spectral_trends_amip6_era5_airsL3_umbc.amip6_100_layertrends.ptemp,nwp_spectral_trends_amip6_era5_airsL3_umbc.amip6_100_layertrends.gas_1,rates,mask); 
  disp(' ')
  disp('computing ERA5    spectral closure'); 
    compute_deltaRH.era5 = const_rh_4608(nwp_spectral_trends_amip6_era5_airsL3_umbc.era5_100_layertrends.stemp,nwp_spectral_trends_amip6_era5_airsL3_umbc.era5_100_layertrends.ptemp,nwp_spectral_trends_amip6_era5_airsL3_umbc.era5_100_layertrends.gas_1,rates,mask); 
  disp(' ')
  disp('computing AIRS L3 spectral closure'); 
    compute_deltaRH.airsL3 = const_rh_4608(nwp_spectral_trends_amip6_era5_airsL3_umbc.airsL3_100_layertrends.stemp,nwp_spectral_trends_amip6_era5_airsL3_umbc.airsL3_100_layertrends.ptemp,nwp_spectral_trends_amip6_era5_airsL3_umbc.airsL3_100_layertrends.gas_1,rates,mask); 
  disp(' ')
  [nansum(compute_deltaRH.amip6.BT_orig(:)-compute_deltaRH.era5.BT_orig(:)) nansum(compute_deltaRH.amip6.BT_orig(:)-compute_deltaRH.airsL3.BT_orig(:)) nansum(compute_deltaRH.amip6.BT_orig(:)-compute_deltaRH.umbc.BT_orig(:))]
end

%{
comment = 'see plot_profile_trends2.m';
the_nwp_trends.airsL3 = nwp_spectral_trends_amip6_era5_airsL3_umbc.airsL3_spectral_rates;
the_nwp_trends.era5 = nwp_spectral_trends_amip6_era5_airsL3_umbc.era5_spectral_rates;
the_nwp_trends.amip6 = nwp_spectral_trends_amip6_era5_airsL3_umbc.amip6_spectral_rates;
if iNorD > 0
...
junk = compute_deltaRH.amip6.BT_orig;
figure(1); clf
  plot(f,nanstd(rates(:,mask)'-(compute_deltaRH.umbc.BT_final(:,mask)'-junk(:,mask)')),...
       ...
       f,nanstd(rates(:,mask)'-(compute_deltaRH.amip6.BT_final(:,mask)'-junk(:,mask)')),...
       f,nanstd(rates(:,mask)'-(compute_deltaRH.era5.BT_final(:,mask)'-junk(:,mask)')))
  plotaxis2;
  hl = legend('UMBC reconstruct','AIRS L3 reconstruct','AMIP6 reconstruct','ERA5 reconstruct','location','best','fontsize',8);
...
figure(1);
  plot(f,nanmean(rates(:,mask)'-(compute_deltaRH.umbc.BT_final(:,mask)'-junk(:,mask)')),...
       ...
       f,nanmean(rates(:,mask)'-(compute_deltaRH.amip6.BT_final(:,mask)'-junk(:,mask)')),...
       f,nanmean(rates(:,mask)'-(compute_deltaRH.era5.BT_final(:,mask)'-junk(:,mask)')))
  plot(f,nanmean(rates(:,mask)'-nwp_spectral_trends_amip6_era5_airsL3_umbc.umbc_spectral_rates(:,mask)'),...
       f,nanmean(rates(:,mask)'-nwp_spectral_trends_amip6_era5_airsL3_umbc.airsL3_spectral_rates(:,mask)'),...
       f,nanmean(rates(:,mask)'-nwp_spectral_trends_amip6_era5_airsL3_umbc.amip6_spectral_rates(:,mask)'),...
       f,nanmean(rates(:,mask)'-nwp_spectral_trends_amip6_era5_airsL3_umbc.era5_spectral_rates(:,mask)'))
  plotaxis2;
...
%       f,nanmean(compute_deltaRH.amip6.BT_final(:,mask)'-compute_deltaRH.amip6.BT_orig(:,mask)'),...
...
%  hl = legend('UMBC reconstruct','AIRS L3 reconstruct','AMIP6 reconstruct','ERA5 reconstruct','AIRS obs','location','best','fontsize',8);
...
figure(3); subplot(121);
           semilogy(nanmean(compute_deltaRH.umbc.final(1:97,mask)'-compute_deltaRH.umbc.orig(1:97,mask)'),playsjunk,...
                    ...
                    nanmean(compute_deltaRH.amip6.final(1:97,mask)'-compute_deltaRH.amip6.orig(1:97,mask)'),playsjunk,...
  ...
  hl = legend('UMBC reconstruct','AIRS L3 reconstruct','AMIP6 reconstruct','ERA5 reconstruct','location','best','fontsize',8); xlabel('d(RH)/dt'); ylabel('p(mb)'); xlim([-0.5 +0.5]);
figure(3); subplot(122);
           semilogy(nanmean(deltaT(1:97,mask)'),playsjunk,...
                    nanmean(nwp_spectral_trends_amip6_era5_airsL3_umbc.airsL3_100_layertrends.ptemp(1:97,mask)'),playsjunk,...
                    nanmean(nwp_spectral_trends_amip6_era5_airsL3_umbc.amip6_100_layertrends.ptemp(1:97,mask)'),playsjunk,...
                    nanmean(nwp_spectral_trends_amip6_era5_airsL3_umbc.era5_100_layertrends.ptemp(1:97,mask)'),playsjunk,...
                    'linewidth',2);
  plotaxis2; set(gca,'ydir','reverse'); ylim([10 1000])
  hl = legend('UMBC reconstruct','AIRS L3 reconstruct','AMIP6 reconstruct','ERA5 reconstruct','location','best','fontsize',8); xlabel('d(T)/dt'); ylabel('p(mb)'); xlim([-0.05 +0.05])

%%%%%%%%%%%%%%%%%%%%%%%%%
figure(4); clf
columnSST.umbc   = ones(101,1)*results(:,6)';
columnSST.airsL3 = ones(101,1)*reshape(airsL3.thestats64x72.stemprate,1,72*64);
columnSST.amip6  = ones(101,1)*amip6.trend_stemp;
...
figure(4); subplot(121);
           semilogy(nanmedian((compute_deltaRH.umbc.final(1:97,mask)'-compute_deltaRH.umbc.orig(1:97,mask)')./(columnSST.umbc(1:97,mask)')),playsjunk,...
                    ...
                    nanmedian((compute_deltaRH.amip6.final(1:97,mask)'-compute_deltaRH.amip6.orig(1:97,mask)')./(columnSST.amip6(1:97,mask)')),playsjunk,...
                    nanmedian((compute_deltaRH.era5.final(1:97,mask)'-compute_deltaRH.era5.orig(1:97,mask)')./(columnSST.era5(1:97,mask)')),playsjunk,...
                    'linewidth',2);
  plotaxis2; set(gca,'ydir','reverse'); ylim([10 1000])
  hl = legend('UMBC reconstruct','AIRS L3 reconstruct','AMIP6 reconstruct','ERA5 reconstruct','location','best','fontsize',8); xlabel('d(RH)/dSKT'); ylabel('p(mb)'); xlim([-10 +10]);
figure(4); subplot(122);
           semilogy(nanmedian((deltaT(1:97,mask)')./(columnSST.umbc(1:97,mask)')),playsjunk,...
                    nanmedian((nwp_spectral_trends_amip6_era5_airsL3_umbc.airsL3_100_layertrends.ptemp(1:97,mask)')./(columnSST.airsL3(1:97,mask)')),playsjunk,...
                    nanmedian((nwp_spectral_trends_amip6_era5_airsL3_umbc.amip6_100_layertrends.ptemp(1:97,mask)')./(columnSST.amip6(1:97,mask)')),playsjunk,...
                    nanmedian((nwp_spectral_trends_amip6_era5_airsL3_umbc.era5_100_layertrends.ptemp(1:97,mask)')./(columnSST.era5(1:97,mask)')),playsjunk,...
                    'linewidth',2);
  plotaxis2; set(gca,'ydir','reverse'); ylim([10 1000])
  hl = legend('UMBC reconstruct','AIRS L3 reconstruct','AMIP6 reconstruct','ERA5 reconstruct','location','best','fontsize',8); xlabel('d(T)/dSKT'); ylabel('p(mb)'); xlim([-2 +2])

epsx = 1e-4;
epsx = 1e-3;
oo1 = find(abs(results(mask,6)) > epsx);
oo2 = reshape(airsL3.thestats64x72.stemprate,1,72*64); oo2 = find(abs(oo2(mask)) > epsx);
oo3 = find(abs(amip6.trend_stemp(mask)) > epsx);
...
figure(4); subplot(121);
           semilogy(nanmean((compute_deltaRH.umbc.final(1:97,mask(oo1))'-compute_deltaRH.umbc.orig(1:97,mask(oo1))')./(columnSST.umbc(1:97,mask(oo1))')),playsjunk,...
                    ...
                    nanmean((compute_deltaRH.amip6.final(1:97,mask(oo3))'-compute_deltaRH.amip6.orig(1:97,mask(oo3))')./(columnSST.amip6(1:97,mask(oo3))')),playsjunk,...
...
figure(4); subplot(122);
           semilogy(nanmean((deltaT(1:97,mask(oo1))')./(columnSST.umbc(1:97,mask(oo1))')),playsjunk,...
                    nanmean((nwp_spectral_trends_amip6_era5_airsL3_umbc.airsL3_100_layertrends.ptemp(1:97,mask(oo2))')./(columnSST.airsL3(1:97,mask(oo2))')),playsjunk,...
                    nanmean((nwp_spectral_trends_amip6_era5_airsL3_umbc.amip6_100_layertrends.ptemp(1:97,mask(oo3))')./(columnSST.amip6(1:97,mask(oo3))')),playsjunk,...
                    nanmean((nwp_spectral_trends_amip6_era5_airsL3_umbc.era5_100_layertrends.ptemp(1:97,mask(oo4))')./(columnSST.era5(1:97,mask(oo4))')),playsjunk,...
                    'linewidth',2);
  plotaxis2; set(gca,'ydir','reverse'); ylim([10 1000])
  hl = legend('UMBC reconstruct','AIRS L3 reconstruct','AMIP6 reconstruct','ERA5 reconstruct','location','best','fontsize',8); xlabel('d(T)/dSKT'); ylabel('p(mb)'); xlim([-2 +2])
