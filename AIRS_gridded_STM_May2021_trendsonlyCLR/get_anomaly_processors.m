function [jobjunk,mapAnomData_to_processor] = get_anomaly_processors(ia_OorC_DataSet_Quantile,iNumAnomTimeSteps,iNumAnomTiles,iNumAnomJobsPerProc,anomalydatafile,JOB)
...
if ia_OorC_DataSet_Quantile(1) == 2 
  ...
  for input_spectrum_number = 1 : iNumAnomTimeSteps * iNumAnomTiles
    ...
    if procnumber == JOB
      fprintf(1,' <<< input_spectrum_number %4i : locallatbin %2i localtimestep ---> procnumber = %4i -----> %4i clustjob = %03i >>> \n',input_spectrum_number,latnumber,iLocalTimeStep,procnumber,JOB);
      junkcnt = junkcnt + 1;
      ...
      if junkprevious ~= procnumber
        fprintf(1,'input_spectrum_number %4i : locallatbin %2i localtimestep ---> procnumber = %4i -----> %4i \n',input_spectrum_number,latnumber,iLocalTimeStep,procnumber);
        junkprevious = procnumber;
      end
    end 
    mapAnomData_to_processor(input_spectrum_number,1) = latnumber;       %% remeber, if "global_and_28"             then latnumber 1 = global,                        latnumber 2:29 are fatbins
    mapAnomData_to_processor(input_spectrum_number,1) = latnumber;       %% remeber, if "global_and_tropics_and_28" then latnumber 1 = global, latnumber 2 = tropics, latnumber 3:30 are fatbins
