...
%cmip6_spectral_olr.olr0_rrtm  = driver_rrtm_no_xsec_nocloud_twoslab_band17only_loop(h,px,0); %%% <<<<<<<<<<<<<<<<<<<<<<<<<<<
cmip6_spectral_olr.olr0_ecRad = superdriver_run_ecRad_rtp_loop_over_profiles(h,px,-1);                
eval(cdRRTMback);
...
%cmip6_spectral_olr.skt_rrtm  = driver_rrtm_no_xsec_nocloud_twoslab_band17only_loop(h,px,0);
cmip6_spectral_olr.skt_ecRad = superdriver_run_ecRad_rtp_loop_over_profiles(h,px,-1);              
eval(cdRRTMback);
...
%cmip6_spectral_olr.planck_rrtm  = driver_rrtm_no_xsec_nocloud_twoslab_band17only_loop(h,px,0);
cmip6_spectral_olr.planck_ecRad = superdriver_run_ecRad_rtp_loop_over_profiles(h,px,-1);              
eval(cdRRTMback);
...
%cmip6_spectral_olr.lapse_rrtm  = driver_rrtm_no_xsec_nocloud_twoslab_band17only_loop(h,px,0);
cmip6_spectral_olr.lapse_ecRad = superdriver_run_ecRad_rtp_loop_over_profiles(h,px,-1);              
eval(cdRRTMback);
...
px = p;
%px.gas_2 = px.gas_2*(1+2.2/400);
  junk = cmip6.trend_gas_1(1:100,:); junk(isnan(junk)) = 1; 
px.gas_1(1:100,:) = px.gas_1(1:100,:) .* (1 + junk(1:100,:));
cmip6_spectral_olr.wv = compute_olr(h,px);
%cmip6_spectral_olr.wv_rrtm  = driver_rrtm_no_xsec_nocloud_twoslab_band17only_loop(h,px,0);
...
figure(71); scatter_coast(p.rlon,p.rlat,50,cmip6_spectral_olr.feedback.planck); caxis([-4 0]*1);  colormap(jet);  title('CMIP6 \lambda_{Planck}')
figure(72); scatter_coast(p.rlon,p.rlat,50,cmip6_spectral_olr.feedback.lapse);  caxis([-5 +5]*1); colormap(usa2); title('CMIP6 \lambda_{Lapse}')
...
cmip6_spectral_olr.feedback.planck_ecRad = cmip6_spectral_olr.planck_ecRad.clr-cmip6_spectral_olr.olr0_ecRad.clr; 
  cmip6_spectral_olr.feedback.planck_ecRad = -cmip6_spectral_olr.feedback.planck_ecRad./cmip6.trend_stemp;
cmip6_spectral_olr.feedback.lapse_ecRad = cmip6_spectral_olr.lapse_ecRad.clr-cmip6_spectral_olr.planck_ecRad.clr;
  cmip6_spectral_olr.feedback.lapse_ecRad = -cmip6_spectral_olr.feedback.lapse_ecRad./cmip6.trend_stemp;
cmip6_spectral_olr.feedback.wv_ecRad = cmip6_spectral_olr.wv_ecRad.clr-cmip6_spectral_olr.olr0_ecRad.clr;
  cmip6_spectral_olr.feedback.wv_ecRad = -cmip6_spectral_olr.feedback.wv_ecRad./cmip6.trend_stemp;
cmip6_spectral_olr.feedback.skt_ecRad = cmip6_spectral_olr.skt_ecRad.clr-cmip6_spectral_olr.olr0_ecRad.clr;
  cmip6_spectral_olr.feedback.skt_ecRad = -cmip6_spectral_olr.feedback.skt_ecRad./cmip6.trend_stemp;

figure(75); scatter_coast(p.rlon,p.rlat,50,cmip6_spectral_olr.feedback.planck_ecRad); caxis([-4 0]*1.5);  colormap(jet);  title('CMIP6 \lambda_{Planck}')
figure(76); scatter_coast(p.rlon,p.rlat,50,cmip6_spectral_olr.feedback.lapse_ecRad);  caxis([-5 +5]*2); colormap(usa2); title('CMIP6 \lambda_{Lapse}')
figure(77); scatter_coast(p.rlon,p.rlat,50,cmip6_spectral_olr.feedback.wv_ecRad);     caxis([-2 +2]*2); colormap(usa2); title('CMIP6 \lambda_{WV}')
figure(78); scatter_coast(p.rlon,p.rlat,50,cmip6_spectral_olr.feedback.skt_ecRad);    caxis([-2 0]*1);  colormap(jet);  title('CMIP6 \lambda_{Skt}')

wonk = cmip6_spectral_olr.feedback.planck_ecRad; wonk(wonk < -10) = NaN; wonk(wonk > 0) = NaN; 
  ns = 500; aslmap(75,rlat65,rlon73,maskLFmatr.*smoothn((reshape(wonk,72,64)'),ns),[-90 +90],[-180 +180]); colormap(jet);  caxis([-4 0]*1.5);  title('CMIP6 \lambda_{Planck}')
wonk = cmip6_spectral_olr.feedback.lapse_ecRad; wonk(wonk < -5) = NaN; wonk(wonk > +5) = NaN; 
  ns = 500; aslmap(76,rlat65,rlon73,maskLFmatr.*smoothn((reshape(wonk,72,64)'),ns),[-90 +90],[-180 +180]);  colormap(usa2); caxis([-5 5]*1);    title('CMIP6 \lambda_{Lapse}')
wonk = cmip6_spectral_olr.feedback.wv_ecRad; wonk(wonk < -5) = NaN; wonk(wonk > +5) = NaN; 
  ns = 500; aslmap(77,rlat65,rlon73,maskLFmatr.*smoothn((reshape(wonk,72,64)'),ns),[-90 +90],[-180 +180]);     colormap(usa2); caxis([-2 2]*1);    title('CMIP6 \lambda_{WV}')
wonk = cmip6_spectral_olr.feedback.skt_ecRad; wonk(wonk < -3) = NaN; wonk(wonk > +3) = NaN; 
  ns = 500; aslmap(78,rlat65,rlon73,maskLFmatr.*smoothn((reshape(wonk,72,64)'),ns),[-90 +90],[-180 +180]);    colormap(jet);  caxis([-2 0]*1.5);  title('CMIP6 \lambda_{Skt}')
...
bad = find(abs(cmip6.trend_stemp) < 1e-4);
junklat = reshape(p.rlat,72,64); junklat = mean(junklat,1);
figure(79); clf
  junk = cmip6_spectral_olr.feedback.planck_ecRad; junk(bad) = NaN; junk = reshape(junk,72,64);  junk = nanmean(junk,1); junk = smooth(junk,5); plot(junklat,junk,'b','linewidth',2); hold on
  junk = cmip6_spectral_olr.feedback.lapse_ecRad; junk(bad) = NaN; junk = reshape(junk,72,64);   junk = nanmean(junk,1); junk = smooth(junk,5); plot(junklat,junk,'g','linewidth',2); hold on
  junk = cmip6_spectral_olr.feedback.wv_ecRad; junk(bad) = NaN; junk = reshape(junk,72,64);      junk = nanmean(junk,1); junk = smooth(junk,5); plot(junklat,junk,'r','linewidth',2); hold on
  junk = cmip6_spectral_olr.feedback.skt_ecRad; junk(bad) = NaN; junk = reshape(junk,72,64);     junk = nanmean(junk,1); junk = smooth(junk,5); plot(junklat,junk,'k','linewidth',2); hold off
ylim([-10 +10]/2); plotaxis2; title('CMIP6 \lambda'); xlabel('Latitude'); hl = legend('Planck','Lapse','WV','SKT','location','best','fontsize',8);

junk = cmip6_spectral_olr.feedback.planck_ecRad; junk(bad) = NaN;
