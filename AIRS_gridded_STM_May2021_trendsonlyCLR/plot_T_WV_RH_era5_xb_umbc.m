...
plotoptions.cx = [-1 +1]*0.15; plotoptions.maintitle = 'dT(ERA5)/dt - dTX/dt'; plotoptions.plotcolors = llsmap5;
plotoptions.yLimits = [10 1000];
plotoptions.yLinearOrLog = -1;
z1 = resultsT';          z1 = reshape(z1,length(pjunk20),72,64); z1 = squeeze(nanmean(z1,2));
z2 = xbT';               z2 = reshape(z2,length(pjunk20),72,64); z2 = squeeze(nanmean(z2,2));
...
plotoptions.cx = 1 + ([-1 +1]*2); plotoptions.maintitle = '1 - (dWVXfrac/dt / dWVXfrac(REA5)/dt)'; plotoptions.plotcolors = llsmap5;
plotoptions.cx = [-1 +1];         plotoptions.maintitle = '1 - (dWVXfrac/dt / dWVXfrac(REA5)/dt)'; plotoptions.plotcolors = llsmap5;
plotoptions.cx = [-1 +1];         plotoptions.maintitle = '(dWVXfrac/dt / dWVXfrac(REA5)/dt) - 1'; plotoptions.plotcolors = llsmap5;
plotoptions.yLimits = [100 1000];
plotoptions.yLinearOrLog = +1;
z1 = resultsWV';          z1 = reshape(z1,length(pjunk20),72,64); z1 = squeeze(nanmean(z1,2));
z2 = xbWV';               z2 = reshape(z2,length(pjunk20),72,64); z2 = squeeze(nanmean(z2,2));
