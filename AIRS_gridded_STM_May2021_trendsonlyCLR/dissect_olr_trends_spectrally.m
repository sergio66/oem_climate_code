...
if ~exist('era5_spectral_olr')
  ...
  elseif iWhich == 4
    ...
    %junk = load(fUMBC,'umbc_spectral_olr');  %% this is in compare_OLR_trend.m
    %junk.iNumYears = 20;
    %junk.strUMBC = 'who knows';
...
figure(6); clf
wahT = squeeze(nanmean(reshape(t_bands.clr-olr0_bands.clr,16,72,64),2));
pcolor(bandcenter,lat64,wahT'); shading interp; colorbar; xlabel('RRTM band center [cm-1]'); ylabel('Latitude'); title([dataset_name '\delta OLR due to \delta (TEMP)']); colormap jet
caxis([-1 +1]*2*1e-2); colormap(usa2);
...
%caxis([-1 +1]*2*1e-2); colormap(usa2);
figure(7); clf
wahO3x = squeeze(nanmean(reshape(o3_bands.clr-olr0_bands.clr,16,72,64),2)) + squeeze(nanmean(reshape(ghg_bands.clr-olr0_bands.clr,16,72,64),2));
pcolor(bandcenter,lat64,wahO3x'); shading interp; colorbar; xlabel('RRTM band center [cm-1]'); ylabel('Latitude'); title([dataset_name '\delta OLR due to \delta (CO2,O3,N2O,CH4)']); colormap jet
caxis([-1 +1]*2*1e-2); colormap(usa2);

figure(8); clf
wahGHG = squeeze(nanmean(reshape(ghg_bands.clr-olr0_bands.clr,16,72,64),2));
pcolor(bandcenter,lat64,wahT'); shading interp; colorbar; xlabel('RRTM band center [cm-1]'); ylabel('Latitude'); title([dataset_name '\delta OLR due to \delta (GHG)']); colormap jet
caxis([-1 +1]*2*1e-2); colormap(usa2);
...
wahALL_1 = squeeze(nanmean(reshape(t_bands.clr-olr0_bands.clr,16,72,64),2)) + squeeze(nanmean(reshape(wv_bands.clr-olr0_bands.clr,16,72,64),2)) + squeeze(nanmean(reshape(skt_bands.clr-olr0_bands.clr,16,72,64),2)) + ...
           squeeze(nanmean(reshape(o3_bands.clr-olr0_bands.clr,16,72,64),2)) + squeeze(nanmean(reshape(ghg_bands.clr-olr0_bands.clr,16,72,64),2));
...
figure(10); clf
%plot(lat64,nansum(wahALL_1,1),lat64,nansum(wahALL_2,1),'linewidth',2); plotaxis2; ylabel('Flux Trend'); xlabel('Latitude'); hl = legend('v1 = missing g4,6, now added','v2 = all'); title(dataset_name)
plot(lat64,nansum(wahALL_1,1),lat64,nansum(wahALL_2,1),'linewidth',2); plotaxis2; ylabel('Flux Trend'); xlabel('Latitude'); hl = legend('v1 = components added','v2 = all'); title(dataset_name)
...
junkT    = (t_bands.clr-olr0_bands.clr) .* wunk;  
junkGHG  = (ghg_bands.clr-olr0_bands.clr) .* wunk;  
...
meanjunkT   = nansum(junkT,2)./nansum(wunk,2);
meanjunkGHG = nansum(junkGHG,2)./nansum(wunk,2);
meanjunkSKT = nansum(junkSKT,2)./nansum(wunk,2);
meanjunkALL = nansum(junkALL,2)./nansum(wunk,2);
%figure(1); clf; plot(bandcenter,meanjunkSKT,'g',bandcenter,meanjunkWV,'b',bandcenter,meanjunkT,'r',bandcenter,meanjunkO3,'c','linewidth',2);
%           hold on; plot(bandcenter,meanjunkSKT+meanjunkWV+meanjunkT+meanjunkO3,'k--',bandcenter,meanjunkALL,'kx-','linewidth',4); hold off
%plotaxis2; hl = legend('SKT','WV','T+CO2','O3','ALLv1 = SUM(4)','ALLv2','location','best','fontsize',10);

figure(1); clf; plot(bandcenter,meanjunkSKT,'g',bandcenter,meanjunkWV,'b',bandcenter,meanjunkT,'r',bandcenter,meanjunkGHG,'m',bandcenter,meanjunkO3,'c','linewidth',2);
           hold on; plot(bandcenter,meanjunkSKT+meanjunkWV+meanjunkT+meanjunkO3+meanjunkGHG,'color',[1 1 1]*0.6,'linewidth',4); hold off
           hold on; plot(bandcenter,meanjunkALL,'kx-','linewidth',4); hold off
plotaxis2; hl = legend('SKT','WV','T','GHG','O3','ALLv1 = SUM(5)','ALLv2','location','best','fontsize',10);
xlabel('RRTM band center [cm-1]'); ylabel('dOLR/dt [W/m2/yr]'); title(dataset_name)
...
%figure(1); clf; plot(bandcenter,meanjunkSKT,bandcenter,meanjunkWV,bandcenter,meanjunkT,bandcenter,meanjunGHG,bandcenter,meanjunkSKT+meanjunkWV+meanjunkT+meanjunkO3+meanjunkGHG,'kx-','linewidth',2);
%plotaxis2; hl = legend('SKT','WV','T','GHG','ALL','location','best','fontsize',10);

punk = cos(lat64*pi/180);
fprintf(1,'%s : add (cosine weighted)  flux trend = sum(T+WV+SKT+O3+GHG) = %8.4f W/m2/yr           sum(all even CH4,N2O) = %8.4f W/m2/yr \n',dataset_name,sum(meanjunkSKT+meanjunkWV+meanjunkT+meanjunkO3+meanjunkGHG),sum(meanjunkALL))
