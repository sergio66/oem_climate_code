...
datime = 2002.75 + (1:iNumAnomTimeSteps)*16/365;
  figure(3); clf; junkjunk = squeeze(btanomD(:,i1231,:));  junkjunk = reshape(junkjunk,72,64,iNumAnomTimeSteps); junkjunk = squeeze(nanmean(junkjunk,1)); 
  ...
  figure(4); clf; junkjunk = squeeze(btanomD(:,iXCh,:)); junkjunk = reshape(junkjunk,72,64,iNumAnomTimeSteps); junkjunk = squeeze(nanmean(junkjunk,1)); 
...
if iHuge < 0
  figure(iOffSet+7); clf
   ...
   P = polyfit(daysSince2002D,nanmean(btanomD,1),1);         fprintf(1,'quick BTtrend = %8.6f K/year \n',P(1)*365)
   P = polyfit(daysSince2002D,nanmean(btanomD.*coslat,1),1); fprintf(1,'wgted BTtrend = %8.6f K/year \n',P(1)*365)
   
   pcolor(2002+daysSince2002D/365,rlat,squeeze(nanmean(reshape(btanomD,72,64,iNumAnomTimeSteps),1))); shading interp; colorbar; colormap(usa2); caxis([-1 +1]*1)
  ...
  PX = polyfit(daysSince2002D,nanmean(btanomD.*coslat,1),1); YPX = polyval(PX,daysSince2002D);
  
  plot(onidd,10*oni,'k',...
       2002+daysSince2002D/365,smooth(100*(nanmean(btanomD,1)),10),'b',...
       2002+daysSince2002D/365,smooth(100*(nanmean(btanomD,1)-YP),10),'r','linewidth',2); xlim([2002 2023]);
  plot(onidd,10*oni,'k',...
       2002+daysSince2002D/365,smooth(100*(nanmean(btanomD,1)-YP),24),'r','linewidth',2); xlim([2002 2023]);
  plot(onidd,10*oni,'k',...
       2002+daysSince2002D/365,smooth(100*(nanmean(btanomD.*coslat,1)-YPX),24),'r','linewidth',2); xlim([2002 2023]);
  plotaxis2;
...
elseif iHuge > 0
  ...
  P = polyfit(daysSince2002D,moo(i1231,:),1);             fprintf(1,'quick BTtrend = %8.6f K/year \n',P(1)*365)
  moox = squeeze(btanomD(:,i1231,:));
  P = polyfit(daysSince2002D,nanmean(moox.*coslat,1),1); fprintf(1,'wgted BTtrend = %8.6f K/year \n',P(1)*365)
  
  pcolor(2002+daysSince2002D/365,rlat,squeeze(nanmean(reshape(moox,72,64,iNumAnomTimeSteps),1))); shading interp; colorbar; colormap(usa2); caxis([-1 +1]*1)
    xlabel('Time'); ylabel('Latitude'); title(['BT' num2str(h.vchan(iChID),'%6.2f') ' cm-1 anomaly']); 
  ...
  PX = polyfit(daysSince2002D,nanmean(moox.*coslat,1),1); YPX = polyval(PX,daysSince2002D);
  
  plot(onidd,10*oni,'k',...
       2002+daysSince2002D/365,smooth(100*(nanmean(moox,1)),10),'b',...
       2002+daysSince2002D/365,smooth(100*(nanmean(moox,1)-YP),10),'r','linewidth',2); xlim([2002 2023]);
  plot(onidd,10*oni,'k',...
       2002+daysSince2002D/365,smooth(100*(nanmean(moox,1)-YP),24),'r','linewidth',2); xlim([2002 2023]);
  plot(onidd,10*oni,'k',...
       2002+daysSince2002D/365,smooth(100*(nanmean(moox.*coslat,1)-YPX),24),'r','linewidth',2); xlim([2002 2023]);
  plotaxis2;
  hl = legend('ONI',['BT' num2str(h.vchan(iChID),'%6.2f') ' cm-1 anomaly'],'location','best');
  hl = legend('ONI',['BT' num2str(h.vchan(iChID),'%6.2f')],'location','best');
...
if iHuge < 0
  if exist('btanonA')
    plot(2002 + daysSince2002D/365,btanomA(2787,:),'b',2002 + daysSince2002D/365,btanomD(2787,:),'r','linewidth',2); %% LatBin = 39, LonBin = 51 >> (39-1)*72 + 51 = 2787
  else
    plot(2002 + daysSince2002D/365,btanomD(2787,:)*0,'b',2002 + daysSince2002D/365,btanomD(2787,:),'r','linewidth',2); %% LatBin = 39, LonBin = 51 >> (39-1)*72 + 51 = 2787
  end
  plotaxis2;
...
if iHuge < 0
  figure(iOffSet+9); clf
  if exist('btanomA')
    plot(2002 + daysSince2002D/365,btanomA(2788,:),'b',2002 + daysSince2002D/365,btanomD(2788,:),'r','linewidth',2); %% LatBin = 39, LonBin = 52 >> (39-1)*72 + 52 = 2788
  else
    plot(2002 + daysSince2002D/365,btanomD(2788,:)*0,'b',2002 + daysSince2002D/365,btanomD(2788,:),'r','linewidth',2); %% LatBin = 39, LonBin = 52 >> (39-1)*72 + 52 = 2788
  end
  plotaxis2;
...
if iSave > 0
  ...
  if iHuge < 0
    ...
    saver = ['save ' fout ' bt1231_D btChID_D btanomD yyD mmD ddD hhD rtimeD fluxanomD    btanomA yyA mmA ddA hhA rtimeA bt1231_A btChID_A fluxanomA   comment ind f_ind'];
  else
    fout = ['/asl/s1/sergio/JUNK/anomaly_ALL_Q' num2str(iQuant,'%02d') '_numyears_' num2str(iNumYears,'%6.2f') '_iNumAnomTimeSteps_' num2str(iNumAnomTimeSteps) '_D.mat'];  %% orig
    fout = ['/asl/s1/sergio/JUNK/anomaly_iQAX_' num2str(iQAX) '_ALL_Q' num2str(iQuant,'%02d') '_numyears_' num2str(iNumYears,'%6.2f') '_iNumAnomTimeSteps_' num2str(iNumAnomTimeSteps) '_D.mat'];
    saver = ['save -v7.3 ' fout ' btanomD yyD mmD ddD hhD rtimeD comment bt1231_D fluxanomD ind f_ind'];
  end
  eval(saver)
end
