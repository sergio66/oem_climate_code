...
for iLatbin = iLS : iLE
  ...
  if i_obsORcal > 0
    ...
    fname = ['/home/strow/Work/Airs/Stability/Data_old/Desc_fits/fit_rcal_lat' num2str(iLatbin) '.mat'];
    fname = ['/home/strow/Work/Airs/Stability/Data/Desc_fits/fit_rclr_lat' num2str(iLatbin) '.mat'];
  end
  a = load(fname);

  %% figure out spanning data
  yyS = 2002; mmS = 9; ddS = 01; yyE = 2018; mmE = 8; ddE = 31; 
  yyS = 2012; mmS = 5; ddS = 01; yyE = 2018; mmE = 4; ddE = 30; 
  ...
  for iChan = 1 : 2645
    ...
    for iDay16 = 1 : length(ia16)-1
      ...
      avg_doy_since2012(iDay16) = mean(iaAllDays(ind));
  ...
  if i_obsORcal > 0
    saver = ['save /home/sergio/MATLABCODE/QUICKTASKS_TELECON/AIRS2CRIS_and_IASI2CRIS/sergio_airs2cris_decon/ANOM_16dayavg_AIRS_2012_nonucal/latbin_' num2str(iAvgNumDays) 'dayavg_' num2str(iLatbin) '.mat avg16_rtime avg_doy_since2012 avg16_btanom comment'];
  elseif i_obsORcal < 0
    saver = ['save /home/sergio/MATLABCODE/QUICKTASKS_TELECON/AIRS2CRIS_and_IASI2CRIS/sergio_airs2cris_decon/ANOM_16dayavg_AIRS_2012_nonucal/latbin_' num2str(iAvgNumDays) 'dayavg_' num2str(iLatbin) '_cal.mat avg16_rtime avg_doy_since2012 avg16_btanom comment'];
  end
  ...
  if iPlot > 0
    figure(2); clf
    pcolor(2012+(avg_doy_since2012)/365,1:2645,avg16_btanom'); colormap jet; colorbar; shading interp;
    caxis([-2 +2]);
...
iLatbin = 20;
  if i_obsORcal > 0  
    fin = ['ANOM_16dayavg_nonucal/latbin_' num2str(iAvgNumDays) 'dayavg_' num2str(iLatbin) '.mat'];
  else
    fin = ['ANOM_16dayavg_nonucal/latbin_' num2str(iAvgNumDays) 'dayavg_' num2str(iLatbin) '_cal.mat'];
  end
  load(fin)
