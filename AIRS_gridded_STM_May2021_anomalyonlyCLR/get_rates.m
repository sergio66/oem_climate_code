...
%     driver.rateset.unc_rates = real(squeeze(b_err_obs(ix,:,2))');
     ...
     driver.rateset.unc_rates = real(squeeze(b_err_desc(ix,iy,:)));
  end

elseif driver.i16daytimestep > 0
  anom = load(driver.rateset.datafile);
  anom.avg16_btanom = anom.bt_anom_all';
  ...
  if driver.i16daytimestep > mmjunk
    ...
    driver.rateset.rates = anom.bt_anom_all(:,driver.i16daytimestep);
    ...
    if iSetType == 1
      disp('  get_rates : unc iSetType = 1 = const')
      anom_noise = ones(size(driver.rateset.rates)) * facx;
    elseif iSetType == 2
    ...
    elseif iSetType == 1 | iSetType == 2
      anom_noise = anom_noise;
    end
