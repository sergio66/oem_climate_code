...
%junk4trp = junk4trp + (junk6trp-junk4trp)/20;
  ...
  if ii > 1
    figure(1); plot(yymm,smooth(junk0,smn),'m',yymmL1C,smooth(junkObsALL,2*smn+1),'b.-',yymmL1C,smooth(junkObsCLR,2*smn+1),'r.-',yymm,rh_plot(ii)*smooth(junk4,smn),'g',yymm,smooth(junk5,smn),'c','linewidth',2); 
      ...
      legend('CLR sarta','Obs ALL','Obs CLR','Const RH','ALL sarta','fontsize',8,'location','best');
      set(gca,'fontsize',10)

    figure(2); plot(yymm,smooth(junk0,smn),'m',yymmL1C,smooth(junkObsALL,2*smn+1),'bd-',yymmL1C,smooth(junkObsCLR,2*smn+1),'ro-',yymm,rh_plot(ii)*smooth(junk4,smn),'g',yymm,smooth(junk5,smn),'c','linewidth',2); 
      plotaxis2; ylabel('Flux [W/m2]'); title([num2str(ii) ' : ' num2str(RRTM_bands(ii-1)) ' to ' num2str(RRTM_bands(ii))]);
      xlim([2020.5 2024.5]); ax = axis; line([ht ht],[ax(3) ax(4)],'color','k','linewidth',2)
      legend('CLR sarta','Obs ALL','Obs CLR','Const RH','ALL sarta','fontsize',8,'location','best');
      set(gca,'fontsize',10)

    figure(3); plot(yymm,smooth(junk0trp,smn),'m',yymmL1C,smooth(junkObsALLtrp,2*smn+1),'bd-',yymmL1C,smooth(junkObsCLRtrp,2*smn+1),'ro-',yymm,rh_plot(ii)*smooth(junk4trp,smn),'g',yymm,smooth(junk5trp,smn),'c','linewidth',2); 
  ...
  if iPrint > 0
    figure(1); sergioprint(['/home/sergio/PAPERS/AIRS/AIRS_STM_Sept2024/Trends/Figs/fluxes_band' num2str(ii,'%02d') '_obs_sarta']);
    figure(2); sergioprint(['/home/sergio/PAPERS/AIRS/AIRS_STM_Sept2024/Trends/Figs/fluxes_band' num2str(ii,'%02d') '_zoom_obs_sarta']);
    figure(3); sergioprint(['/home/sergio/PAPERS/AIRS/AIRS_STM_Sept2024/Trends/Figs/fluxes_band' num2str(ii,'%02d') '_zoom_topics_obs_sarta']);
  end   
