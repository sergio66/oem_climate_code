...
if iSmooth >= 1
  junk = smoothn(reshape(junk,72,64)',1);
elseif iSmooth == 0.5
  junk = smoothdata(reshape(junk,72,64)',"movmean",5);
elseif iSmooth <= 0
  junk = reshape(junk,72,64)';
end
aslmap(86,rlat65,rlon73,junk,[-90 +90],[-180 +180]); colormap(llsmap5); caxis([-1 +1]*2); 
...
aslmap(87,rlat65,rlon73,junk,[-90 +90],[-180 +180]); colormap(llsmap5); caxis([-1 +1]*0.15); 
...
aslmap(88,rlat65,rlon73,junk,[-90 +90],[-180 +180]); colormap(llsmap5); caxis([-1 +1]*0.01); 
title('ERA5 night LowerTrop dT/dt .* dmmw/dt');

%%%%%%%%%%%%%%%%%%%%%%%%%

if ~exist('ceres_ilr')
  disp('OHOH no ceres ilr')
elseif exist('ceres_ilr')
  junk = ceres_ilr.ceres_trend.trend_sfc_lw_clr_4608;
  if iSmooth >= 1
    junk = smoothn(reshape(junk,72,64)',1);
  elseif iSmooth == 0.5
    junk = smoothdata(reshape(junk,72,64)',"movmean",5);
  ...
  aslmap(89,rlat65,rlon73,[-90 +90],[-180 +180]); colormap(llsmap5); caxis([-1 +1]*0.5); title('CERES dILR/dt');
  ...
  if exist('fERA5_night')
    if isfield(fERA5_night,'ilr_adj')
      ...
      figure(104); clf
      aslmap(104,rlat65,rlon73,junk,[-90 +90],[-180 +180]); title('AIRS\_RT d(ILR)/dt (W/m2/yr)'); caxis([-1 +1]*0.5); colormap(llsmap5)
      ...
      junk = deltaILR_ST.*(1 + co2_ilr_adjustx);
      if iSmooth >= 1
        junk = smoothn(reshape(junk,72,64)',1);
      elseif iSmooth == 0.5
        junk = smoothdata(reshape(junk,72,64)',"movmean",5);
      ...
      aslmap(105,rlat65,rlon73,junk,[-90 +90],[-180 +180]); title('CERES d(ILR)/dt (W/m2/yr)'); caxis([-1 +1]*0.5); colormap(llsmap5)
  
      figure(106); clf
      junk = fERA5_night.ilr_Rld;
      if iSmooth >= 1
        junk = smoothn(reshape(junk,72,64)',1);
      elseif iSmooth == 0.5
        junk = smoothdata(reshape(junk,72,64)',"movmean",5);
      ...
      aslmap(106,rlat65,rlon73,junk,[-90 +90],[-180 +180]); title('ERA5 d(ILR)/dt (W/m2/yr)'); caxis([-1 +1]*0.5); colormap(llsmap5)
